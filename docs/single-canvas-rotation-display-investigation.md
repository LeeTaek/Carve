...
- R23 수정 뒤 전량 재확인: **304개** — Swift Testing 302 + XCTest 2, 실패 0. 기준선이 그만큼 올라갔다 (설계 §19-4-2).
...
| 1'' | flag off 직후 화면 밖 절의 표시 | ❌ **미해결** — 토글 off 시점에 화면 밖이던 절의 잉크가 안 보인다. **스크롤로는 안 살아나고 회전 또는 다른 장 왕복 후에야** 나타난다. **R23 수정 뒤에도 그대로 재현되는 별개 결함.** 런북 §8-7 **R24** |
