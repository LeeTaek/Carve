...
| 1' | 같은 항목의 **N-Canvas 왕복** | ❌ **실패** — 표시는 정상이나 **flag off 로 장을 열기만 해도 v3 절이 전부 v2 로 강등되고 metadata 가 삭제된다.** 획은 잃지 않는다. 원인·영향은 런북 §8-7 **R23** (설계 §10-3 위반). ⚠️ **D9 H 와 무관한 선재 결함**이며 `ef053111` 이 만든 것이 아니다 |
| 1'' | flag off 직후 화면 밖 절의 표시 | ❌ **실패** — 회전(레이아웃 재계산) 전까지 잉크가 안 보인다. 스크롤로는 안 살아난다. 런북 §8-7 **R24** |
