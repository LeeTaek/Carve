...
작성: 2026-09-16 · 갱신: 2026-09-16 · 목표: 2.0.0 · 상태: **계획 확정 / 선행(OLD 빌드)·단계 L 수행(MIG-L1 은 화면 확인 보류) · 단계 A~D 미수행**
...
| MIG-L1 | 실기기의 **실사용 store 를 그대로 관측**한다. 왕복을 새로 만들지 않는다 — TestFlight 로 2.0.0 을 설치한 시점에 이미 일어났다 | 마이그레이션 결과·좌표 형식 분포·행 규모. **수행 완료(데이터) · 화면 확인 미수행** — §5-1 |
...
전후 원본·메타데이터·계보 비교 / 화면 확인:
  ...
  · **화면 확인 미수행** — 사람이 기기를 봐야 한다
크래시·오류 / 로그·스크린샷 경로: 없음 / —
관찰 시간 / 종료 코드 / 미수행 단계: — / — / 화면 확인, CloudKit 전부
판정: 통과(데이터) / 화면은 판정 보류
제한과 다음 조치: store 메타데이터만 집계했고 잉크 내용은 열지 않았다. **데이터가 온전한 것과 화면에
  제자리로 그려지는 것은 별개다.** legacy 가 많은 장(최대 56행)과 히스토리가 깊은 절(최대 16행)을
  눈으로 확인해야 한다.
```
