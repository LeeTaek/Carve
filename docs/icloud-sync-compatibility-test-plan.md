...
**사용자에게 필요한 것(2026-09-22 기준):** dut(4419ABC7…)와 B(FD20B387…)는 같은 샌드박스 계정에 붙어 있고 두 저장소의 계정 식별도 같다. 그런데 **재부팅 뒤 자격 증명이 무효(134400)** 라 미러링이 setup 에서 멈춘다(F48) — **설정 앱에서 그 계정 암호를 다시 넣어야** G1 ~ G4 를 돌릴 수 있다. 그 뒤 `work/sep/scripts/sep-gate-rounds.sh` 를 돌린다(약 25 ~ 40분). G5 의 계정 전환(앱 끈 채 → 켠 채 → 오프라인)은 그다음 따로 요청한다. 앞 세션(다른 맥)의 표본은 이 맥에 적용되지 않으므로 G1 ~ G3 은 이 맥의 dut 저장소(`BibleDrawing` 5행 · 대응 5)에서 새로 뜬다.
...
**하지 못한 것:** 같은 계정 재연결이 필요한 G1 ~ G4 · 로그인된 기기에서의 게이트 보류(F40 의 반대) · 게이트 뒤 연결의 setup/import/export — dut 의 자격 증명이 무효라 멈췄다(F48). 실행 스크립트로 준비해 뒀다(§3-3).
...
**자격 증명 무효로 같은 계정 시험을 못 함 (2026-09-22).** 재부팅 뒤 dut · B 의 샌드박스 계정이 유효한 자격 증명을 잃었다(F48). 암호 재입력은 사용자 조작이라 G1 ~ G4 · `gate-hold` · `gate-connect` 는 준비만 했다.
