...
| ID | 절차 | 확인할 결과 |
|---|---|---|
**클라이언트 두 개를 어떻게 만드나.** 실기기 두 대가 없으므로 **독립된 저장소를 가진 클라이언트 두 개**를 만들어야 한다. iPad 시뮬레이터 두 대에 OLD 와 NEW 를 각각 유지하는 구성을 먼저 시험한다(CK-A0 에서 양방향 전송이 실제로 되는지부터 확인).

⚠️ **시뮬레이터는 개별 오프라인을 만들 수 없다.** `simctl status_bar --wifiMode` 는 **상태바 아이콘만** 바꾸고 실제 연결은 호스트 네트워크를 그대로 쓴다. `simctl` 에 네트워크 차단 옵션이 없다. 그래서 오프라인 편집은 이렇게 만든다.

> 호스트 Wi-Fi 를 끈다 → 두 시뮬레이터 모두 오프라인 → 각자 편집 → Wi-Fi 복구 → **앱을 하나씩 순서대로 실행**

앱이 종료돼 있으면 동기화도 돌지 않으므로 **실행 순서가 곧 재연결 순서**가 된다. 다만 이것도 검증 전 가설이므로 CK-A0 에서 함께 확인하고, 실패하면 실기기 한 대(비행기 모드)와 시뮬레이터 한 대의 조합을 검토한다. **앱을 종료한 것만으로 오프라인이 보장된다고 전제하지 않는다.**

⛔ **U4 검증에 DOWN-L2 상태의 OLD 를 쓰지 않는다.** V1 폴백을 탄 저장소는 `CD_DrawingVO` 를 만들므로 서버의 `CD_BibleDrawing` 을 **애초에 건드리지 않는다.** 그 결과 원본 레코드의 필드가 그대로인 것을 "구버전이 미지 필드를 보존했다" 로 **오판할 수 있다.** U4 를 보려면 앱을 지우고 **깨끗한 V3 저장소로 시작해 CloudKit import 를 받은 OLD** 여야 한다. 이때도 확인되는 것은 "전체 가져오기 이후 수정" 경로이지 기존 클라이언트의 증분 동기화가 아니다.
...
⚠️ **OLD 를 깔 때마다 그 기기의 NEW 로컬 데이터는 사라진다(F7).** V5 store 를 OLD 가 열지 못하고 V1 스키마로 갈아치우며, 그 과정에 경고도 크래시도 없다. 그러므로 한 기기에서 OLD ↔ NEW 를 번갈아 쓰는 시간 분할은 **매번 로컬이 초기화된 상태에서 시작**한다고 전제한다. 그 결과 CloudKit 변경 토큰·존 구독도 함께 사라져 매 회차가 "처음부터 전체 import" 가 되므로, 증분 동기화 경로는 이 방식으로 관측되지 않는다. 또한 **V1 스키마가 된 store 가 CloudKit 에 붙었을 때의 거동이 미확인**이다 — 서버에는 `CD_BibleDrawing` 이 있는데 클라이언트는 `CD_DrawingVO` 를 기대한다. A2 이전에 이것부터 관측한다.
