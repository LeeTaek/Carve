...
| 지적 | 상태 |
|---|---|
| 엔티티 이름 `{DrawingVO}` 만으로 1.0.x 로 판정했다 — 필요조건을 충분조건으로 썼다 | ✅ 코드 — 확인된 1.0.x 모양 두 가지(1.0.0~1.0.3 · 1.0.4~1.0.7, `UnversionedDrawingStore`)와 **해시가 맞을 때만** V1 폴백한다. ⬜ 반례 테스트(이름만 같은 `DrawingVO` 저장소 → 막힘 · 파일 불변)와 1.0.0~1.0.3 모양의 이관 테스트는 아직 없다. 1.0.0 의 `let` 선언이 같은 해시인지는 미확인 |
| "파일을 건드리기 전에 판별" · "지우거나 바꾸지 않았어요" 는 구현보다 강하다 — 앱 스키마로 여는 첫 시도는 이미 한 뒤다 | ✅ 코드 주석 · 문서를 "V1 폴백 전에" 로 좁혔다. ⬜ **화면 안내 문구는 1차 그대로다**(`LaunchProgressView` — "지우거나 바꾸지 않았어요", `unknownVersion` 에 업데이트만 제시). 보장 범위("필사를 지키려고 시작을 멈췄어요") · 앱 삭제 금지 · 문의 경로(`UserFeedback.feedbackAddress`)로 바꾼다. `unknownVersion` 은 더 새 앱만이 아니라 버전 없이 고친 스키마도 포함한다 |
| 진입 대기 효과와 진입 직전 재검사가 검증되지 않았다 — 대체 컨테이너는 저장을 거절하지 않는다 | ✅ 코드 — 진입 대기 효과를 취소할 수 있게 하고 막힘 · 재실행 요구에서 끊는다. 코디네이터가 `.syncCompleted` 에서 `launchRoute` 를 다시 확인한다. ⬜ App 모듈에는 테스트 타깃이 없다 — Domain 에 "진입 결론은 뒤에 온 이벤트 · 대기로 막힘 · 재실행 요구가 되지 않는다(반대도)" 테스트를 더한다 |
| "바뀐 코드는 실패 때만 돈다" 는 부정확하다 — 정상 경로도 로더와 진입 판정을 거친다 | ⬜ 수정 빌드로 정상 V5 열기 · 새 필사 저장 · 재실행 조회 회귀(시뮬레이터 손가락 필기)를 하지 않았다. 동기화 회귀는 샌드박스 계정 기기가 필요하다 |
