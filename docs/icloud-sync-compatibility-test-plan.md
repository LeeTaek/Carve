...
작성: 2026-09-16 · 목표: 2.0.0 · 상태: **계획 확정 / 전체 미수행**
...
1. 설계 문서와 이 계획을 읽고 현재 작업 트리·다른 작업 변경을 확인한다.
2. 시험 환경·두 iPad·OLD 재현 가능성을 확인한다. 부족하면 그 항목은 보류하고 독립적인 순수 규칙 테스트를 진행한다.
...
설계 완료는 이 계획의 확정을 뜻한다. **구현 완료·데이터 보전 검증·출시 승인은 별개**다. 실제 실행 결과를 남기기 전까지 이 문서의 모든 케이스는 미수행이다. 다른 세션의 구현을 이번 문서 작업에서 시작하지 않는다.
