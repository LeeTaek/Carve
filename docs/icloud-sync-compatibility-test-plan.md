...
기존 필기 수정의 F29 (주입)
  ...
  판정(주입 경로 포함): ②-2 는 1차의 세 실패 가운데 **F29(전송 전 편집 유실) · F30(로그아웃 필기의 자동 귀속)을 막았다.**
    초안은 계정 전환 · 로그아웃을 건너 남고, 보이기만 하는 초안은 귀속을 올리지 않는다. 남은 제한: 보이지 않는 초안의 개수 안내 ·
    복구 진입점이 없다(④). 후보 A(미러링 식별)는 이번에도 채택 판단에 쓰지 않았다
  미수행: 오프라인 전환 · 앱을 켠 채 로그아웃 · 1차-2(켠 채 전환) · 인계 조작(④) · 무계정 상태의 즐겨찾기 · 위젯 차단(⑥, 절 메뉴는
    시뮬레이터 자동 조작으로 열리지 않아 사람 손이 필요하다) · 단계 사이 종료(⑤)
```
