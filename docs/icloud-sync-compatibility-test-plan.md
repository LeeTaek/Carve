...
| SEP-1 | **V3 실저장소 판독과 전체 마이그레이션** — OLD 빌드(`49f2dc27` · 1.3.0 · V3)로 만든 표본 넷: ① 무계정(한 번도 로그인 안 함) ② 전송 완료 ③ 미전송(연결 없이 쓴 행) ④ 혼합(전송 완료 + 로그아웃 뒤에 쓴 행) | V3 → V4 → V5 → V6 를 지난 뒤에도 **기본 키 ↔ 대응**이 유지되는가 · 세 값 판정이 표본마다 기대대로 나오는가 · 엔티티 3종이 구분되는가(기본 키가 같아도 합치지 않는가) | **✅ 2026-09-21 수행 — 기본 키 · 대응 유지(F39), 판정은 표본마다 기대대로.** 표본은 ①(무계정, 하네스가 넣은 V3 행)과 ④(OLD 의 미러링이 받은 7행 + 하네스 3행). OLD 가 **화면에서 직접 쓴** 행은 아직 없다(터치 자동화가 이 맥에서 안 됨). 기록은 §5-1 SEP-1. 원래 기준: 표본 ①③ 은 「검증된 대응 없음」, ② 는 「대응 있음」, ④ 는 행별로 갈린다 |
| SEP-2 | **삭제 전파 (가장 중요)** — `.none` 으로 연 저장소에서 행을 지우고 다시 연결한다. 두 갈래로 나눈다: **같은 계정 재연결** · **계정 변경**. 각 갈래에서 **대응 있는 행**과 **대응 없는 행**을 따로 지운다 | 서버 레코드 ID 목록(전 · 후) · **persistent history 트랜잭션** · 대응 테이블 · 다른 기기(B)에서 본 결과. `.none` 열기에서 **이력 추적을 끌 수 있는지**도 함께 본다 | **✅ 2026-09-21 수행 (같은 계정 재연결 갈래) — 대응 있는 행의 삭제는 전파됐고(F37), 대응 없는 행의 연결 전 삭제는 흔적이 없었다(F38).** D1 (가) 확정. 서버 쪽 B 확인과 계정 변경 갈래는 남았다. 기록은 §5-1 SEP-2. 원래 기준: 대응 없는 행의 삭제가 전파되면 C14 의 판정 전제가 깨진 것이다 → C14 「SEP-1 · SEP-2 가 실패하면」 의 **격리 + 새 동기화 저장소**로 옮긴다. F25 의 추정(행 삭제와 식별 변경의 순서)도 이 시험에서 함께 관측한다 |
...
**남은 것:** 계정 변경 갈래(같은 손질 뒤 다른 계정으로 연결) · **SEP-1**(진짜 V3 무계정 저장소 — 식별 키 없음 · 오래된 이력).
...
②(전송 완료만) · ③(미전송만)은 ④ 의 부분집합이라 따로 만들지 않았다. **OLD 가 화면에서 직접 쓴 행은 없다** — 터치 자동화 도구가 이 맥에서 동작하지 않았다. 필기 내용은 합성이지만 판정 · 마이그레이션에는 관계없다.
...
**남은 것:** OLD 가 화면에서 쓴 행(사용자가 그린다) · 계정 변경 갈래.
