...
> **2026-09-16 WIDGET 구현에서 정했다.** D-W1(다크) 은 종이 유지 — 배경을 종이색으로 명시하고 뷰에 라이트 `colorScheme` 을 넣는다. D-W2(이미지 규격) 는 사진용 원본을 주지 않고 **필기 PNG 를 560 × 360pt 안으로** 줄여 담는다. 나머지 결정(선택 범위 · 패밀리 · 진입점)은 [로드맵](./release-2.0.0-roadmap.md) §4 「필사 위젯」 에 있다.
