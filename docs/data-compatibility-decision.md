...
작성·갱신: 2026-09-09 · 브랜치: `codex/canvas-compatibility-plan` · 상태: **보류 종료 — U1 확정 · D2·D6 검증 이월(기기 확보 대기) · D3 은 SCOPE 에서 결정** (§5)
...
| **D2** | R27 대응 | ⏸ **검증 이월.** 기기 확보 대기로 기록한다. 미확인 상태를 유지하며 통과로 바꾸지 않는다 |
| **D6** | U4 확인 (구버전 쓰기의 V4 필드 보존) | ⏸ **검증 이월.** D2 와 같은 사유·같은 절차 |
| **D3** | R28 대응 | ⏸ **알려진 위험으로 유지.** 대응 방식은 **SCOPE(출시 범위 확정) 시점에 결정**한다. 위험 수용이 결정된 상태가 아니다 |
...
⚠️ **이 결정은 검증이 아니다.** R27 · R28 · U4 는 여전히 미확인이며, 로드맵 §7 체크리스트의 "결정 필요 상태를 출시까지 방치하지 않음" 은 **SCOPE 시점에 다시 판단해야 한다.**
