...
작성·갱신: 2026-09-09 · 갱신: 2026-09-17 · 브랜치: `codex/canvas-compatibility-plan` · 상태: **보류 종료 — U1 확정 · D2·D6 검증 이월(기기 확보 대기) · D3 은 SCOPE 에서 결정** (§5) → **2026-09-17 시뮬레이터 왕복: U4(D6) 관측 범위에서 보존 · R28 실측 · R27(D2) 재현 · 서로 모르는 편집은 나중 쪽이 덮어써 유실** (§5-2)
...
이 문서는 **코드·git 조사와 기존 문서 대조**의 결과다. 실기기 검증이나 CloudKit 콘솔 조회는 수행하지 않았다. §2 의 미확인 항목을 확인하기 전에는 §4 의 판단을 확정으로 쓰지 않는다. 2026-09-17 의 시뮬레이터 왕복 결과는 §5-2 에 따로 적었다.
...
| **U4** | CloudKit 이 구버전 클라이언트의 쓰기에서 모르는 필드를 보존하는가 | ✅ **관측 범위에서 보존 (2026-09-17, §5-2)** — 1.3.0 이 편집해 올린 뒤에도 서버의 `CD_rowUUID` · `CD_layoutMetadataData` 가 해시까지 그대로였다. 시뮬레이터 2대 · 1회 · 쓰기 경로 두 가지로 한정. (이전 판정: ❓ **미확인 — U1 확정으로 중요도 상승.** 보존하지 않으면 출시본이 편집한 행에서 `rowUUID`·`layoutMetadataData` 가 사라진다. `b68b6101` 을 "additive schema" 라 부른 전제가 이것이다) | 기기 2대 왕복 (런북 §8-7 절차) → 시뮬레이터 2대 왕복 + 서버 레코드 조회 도구 ([테스트 계획](./icloud-sync-compatibility-test-plan.md) CK-A3) |
...
| 선택지 | 내용 | 비용 |
|---|---|---|
| **A. 기기 2대로 확인** | 런북 §8-7 절차로 절당 행 중복이 실제로 생기는지 관측 | 세션 1회. ⏸ **2026-09-09 현재 한 대를 확보하지 못해 이월** (§5 D2) |
...
**권고했던 것은 A** 였으나 기기 미확보로 **이월했다 (§5 D2).** 기기가 확보되면 A 를 먼저 하고, 그 결과가 C 의 필요 여부를 정한다. 그때까지 R27 은 **미확인**이다.
...
**권고: A + B 조합을 기본선으로, C 의 실현 가능성을 COMPAT-1 에서 한 번 검토.** C 가 싸게 되면 A 의 "감수" 범위가 줄어든다. ⏸ **2026-09-09 결정: 알려진 위험으로 유지하고 대응 방식은 SCOPE 에서 정한다 (§5 D3).** 위험 수용이 결정된 것이 아니다.
...
## 5. 결정 결과 — 2026-09-09 사용자 결정으로 **보류 종료**

기기 한 대를 현재 확보하지 못해 왕복 검증이 불가능하다. **검증이 필요한 항목은 이월하고, 검증 없이 정할 수 있는 것만 정한 상태로 이 항목을 닫는다.**

| # | 항목 | 결정 (2026-09-09) |
|---|---|---|
| ~~D1~~ | 출시본 확인 | ✅ **완료** — 1.3.0 (177) · `49f2dc27` · 스키마 V3 (§3) |
| **D2** | R27 대응 | ⏸ **검증 이월.** 기기 확보 대기로 기록한다. 미확인 상태를 유지하며 통과로 바꾸지 않는다. → ⚠️ **2026-09-17 시뮬레이터로 재현** — 행이 둘이 되고 1.3.0 필기가 두 기기에서 가려진다 (§5-2). 통과가 아니라 위험 확인이며 대응은 SCOPE 에서 정한다 |
| **D6** | U4 확인 (구버전 쓰기의 V4 필드 보존) | ⏸ **검증 이월.** D2 와 같은 사유·같은 절차 → ✅ **2026-09-17 관측: 보존** (§5-2, 범위 한정) |
| **D3** | R28 대응 | ⏸ **알려진 위험으로 유지.** 대응 방식은 **SCOPE(출시 범위 확정) 시점에 결정**한다. 위험 수용이 결정된 상태가 아니다. **2026-09-17 실측으로 메커니즘을 확인했다** (§5-2) — 대응 결정은 여전히 SCOPE |
| **D4** | 번역본 귀속 | ▶ **정책부터 정리한다.** 기존 한국어 데이터와 섞이지 않는 귀속 정책을 먼저 문서로 정하고, **실제 저장 경로 변경은 D2·D6·U2·U3 확인 결과에 맞춰 진행**한다 |
| D5 | BIBLE-EN 의 2.0.0 포함 여부 | D4 정책과 확인 결과에 따름. SCOPE 에서 확정 |

**이월의 조건:** D2 · D6 는 기기 두 대가 확보되는 즉시 런북 §8-7 절차로 **한 세션에서** 확인할 수 있다. U2(`CD_translation` 승격 여부)는 기기가 필요 없는 CloudKit Dashboard 조회 1회이므로 별도로 먼저 처리할 수 있다.

⚠️ **이 결정은 검증이 아니다.** R27 · R28 · U4 는 여전히 미확인이며, 로드맵 §7 체크리스트의 "결정 필요 상태를 출시까지 방치하지 않음" 은 **SCOPE 시점에 다시 판단해야 한다.** (2026-09-17: U4 는 관측 범위에서 보존, R28 은 실측, R27 은 재현 — §5-2)

### 5-1. ⚠️ UI-2 의 "지우기" 가 검증 범위를 넓힌다 (2026-09-09)

UI-2 에서 절 롱탭 메뉴에 **"지우기"(보관 후 초기화)** 가 들어갔다. 지우기는 삭제가 아니라 현재 필사를
**보관 행으로 남기고 활성 행을 비우는** 동작이므로, **절당 행이 의도적으로 하나씩 늘어난다.**

이 전까지 이 앱은 회차가 쌓이지 않는 설계였다 — 편집은 기존 행을 갱신했고, "이전 필사 보기" 는 이력이
아니라 데이터 생존 확인용 안전망이었다 (런북 D9-5-4 · 5-5 가 "판정 불가" 로 남은 이유). **지우기는 이
앱에서 이전 기록을 실제로 쌓기 시작하는 첫 기능이다.**

D2 · D6 를 나중에 확인할 때 아래를 범위에 포함한다.

| 항목 | 왜 |
|---|---|
| **R27** — 절당 행 중복 | 행이 늘어나는 방향이라 중복 여부를 가릴 표면이 넓어진다. 보관 행과 두 기기 중복 행을 구별할 수 있어야 한다 |
| **U4** — 구버전 쓰기의 V4 필드 보존 | 보관 행은 `rowUUID` 로 식별한다. 출시본(V3)은 그 필드를 모르므로, 보존되지 않으면 재시도 시 보관 행이 늘어날 수 있다 |
| **CloudKit 왕복** | 보관 행 생성과 활성 행 비우기는 **로컬 한 트랜잭션**이지만 CloudKit 에는 레코드 2개로 따로 전파된다. 한쪽만 도착한 중간 상태를 다른 기기가 볼 수 있다 (설계 §8-6 의 알려진 한계) |

⚠️ **이것은 새 위험 판정이 아니라 기존 미확인 항목의 범위 확대다.** R27 · R28 · U4 의 등급은 그대로 미확인이다.

### 5-2. 시뮬레이터 왕복 결과 (2026-09-17)

[테스트 계획](./icloud-sync-compatibility-test-plan.md) §5-1 의 CK-A0~A5 를 요약한다. 환경은 시뮬레이터 2대(CK-A4c 는 2.0.0 을 하나 더 둔 3대, 모두 iPad mini (A17 Pro) · iOS 26.2) · 샌드박스 계정 · dev 컨테이너(Development 추정)다. OLD 는 `49f2dc27` 을 현재 SDK 로 재빌드하고 입력 수단 2줄만 바꾼 것, CURRENT 는 `2c124cb3` 이다. 서버 값은 앱에 주입한 읽기 전용 조회 도구로 읽었다 — 샌드박스 계정의 private DB 는 콘솔에서 보이지 않는다.

| 항목 | 관측 | 판정 |
|---|---|---|
| **U4** | 1.3.0 이 1절(지우개) · 4절(획 추가)을 편집해 올린 뒤에도 서버의 `CD_layoutMetadataData`(1 · 4절) · `CD_rowUUID`(4절)가 해시까지 그대로였다. 2.0.0 도 받은 뒤 로컬에 유지했고 다시 올리지 않았다 | ✅ **관측 범위에서 보존.** 행 삭제 · 동시 편집 · Production 은 미관측 |
| **R28 보기** | 1.3.0 은 v3 행을 레이아웃 보정 없이 그린다. 1절은 약 38pt(한 줄쯤) 위로 올라가 위쪽이 잘렸고, 4절은 아래쪽이 잘렸다. 데이터는 그대로다 | ⚠️ **재현** — 표시 결함 |
| **R28 편집** | 1.3.0 은 남은 획을 덮어쓰지 않는다(점 · 획 변환 동일). 새 획만 변환 없이 1.3.0 절 로컬 좌표로 더하고 `drawingVersion` 은 3 으로 남긴다. 그래서 2.0.0 에서 그 획만 1.3.0 화면 기준보다 약 38pt 아래에 놓인다(수치로 추정) | ⚠️ **메커니즘 확인.** §3-3 과 설계 R28 의 "좌표를 캔버스 로컬로 덮어쓴다" 는 **새 획에 한정**된다. 한 행에 좌표계가 다른 획이 섞이므로 §4-2 C 의 복구는 행 단위 판정으로는 부족하다(추정) |
| v1 → v3 전환 | 2.0.0 은 1.3.0 행을 편집하면 같은 레코드를 v3 로 올리고 `layoutMetadataData` 를 붙이되 `rowUUID` 는 발급하지 않는다. 남은 획에는 획 변환(`ty −38`)을 붙인다 | 설계의 legacy 행 원칙과 일치. rowUUID 없는 v3 행이 생긴다 |
| **편집 충돌** (CK-A4b · CK-A4c) | 켜 둔 두 기기가 서로 모르고 같은 절을 고치면(켜 둔 앱이 재실행 전까지 가져오지 않아 쉽게 생긴다 — 푸시 권한 부재 탓으로 추정) 나중에 올린 쪽이 절 필기 전체를 덮어쓴다. 1.3.0 ↔ 2.0.0 두 순서와 2.0.0 끼리 두 순서 모두, 먼저 쓴 필기가 서버와 두 기기에서 사라졌다. 충돌 오류 · 안내는 없었다. 1.3.0 이 나중인 경우는 `drawingVersion` 을 3 → 1 로 되돌리고 낡은 `layoutMetadataData` 를 남겼다 | ⛔ **유실 재현 — 2.0.0 끼리도 같다.** 혼재 버전만의 문제가 아니다. 1.3.0 끼리(기존 출시본에도 있던 동작인지)는 미시험 — [동기화 정책](./icloud-sync-and-backup-policy.md) §4-2 · §5 의 실측 근거 |
| **R27** (CK-A5) | 빈 5절에 A(1.3.0)가 먼저, 그것을 모르는 B(2.0.0)가 나중에 썼다. id · 레코드가 서로 다른 두 행이 생겼고 병합 · 정리는 없었다. **두 기기 모두 2.0.0 행만 보여 주고**, A 의 필기는 저장소에 남았지만 보이지 않는다 | ⚠️ **재현** — 중복 행, 1.3.0 필기 가려짐. 반대 순서와 이전 필사 보기에서의 노출은 미확인 |

38pt 는 이 기기 · 세로 · 기본 본문 설정 레이아웃의 값이다. 1회 관측이므로 통과를 일반화하지 않는다([테스트 계획](./icloud-sync-compatibility-test-plan.md) §4-1).
...
- 실기기 검증 · CloudKit 콘솔 조회 · 구버전 왕복 테스트 (U2 · U3 · U4 미수행) — 2026-09-17 에 U4 는 시뮬레이터 왕복으로 관측했다(§5-2). U2 · U3 는 여전히 미수행
