...
| 7 | 합성 프로브(`compose` HUD 줄, `b25cd1bc`) 추가 · **R18** 기록. `leg`/`mism` 은 **마지막 합성**의 결과라 편집 직후에 줄지 않는다(장을 나갔다 와야 갱신) — 여기서 세 번 헛짚었다 |
| 8 | **D9 H 원인 분리** — 데이터는 도착하는데 화면만 낡음. rev.7 의 **"E-4 는 결함 아님" 판정을 되돌림**(§8-7) |
| 9 | **D9 H 정식 수정 + 실기기 A/B 통과** (`f64a7d79`) — 인자 없이 3왕복 정상, `-CanvasReuseStrokesOnApply` 로 결함 재현. **잔여 검증이 남아 종결 아님** |
...
| `Δ max` | **0.00** (초록, tol 1.00pt). HUD 에 `slack` 줄이 있으면 대신 **`Δ(-slack) max` 0.00** | 0 이 아니면 **레이아웃보다 측정 경로를 먼저 의심** (AGENTS.md — 중첩 호스팅의 `.named` → global 대체). ⚠️ **rev.6 — 이 값은 R16 계열을 잡지 못합니다.** D9-0-d 를 함께 도십시오. ⚠️ **`slack` 줄이 뜨면 `Δ max` 가 0 이 아닌 것이 설계상 정상입니다** — 저장 당시 줄 수가 지금보다 많은 필사 절(설계 §6-3 여유)만큼 뒤 절이 계단으로 밀립니다 (2026-09-14 시편 119편: v1·v2·v4 +1, `Δ max 195.63`). 단일 Canvas 에서는 그만큼 잉크·필기 귀속이 텍스트 행과 실제로 어긋나 있습니다 |
...
| 백업 시점의 App Store 판 버전 | ❓ 백업 당시는 **미기록**. ✅ 다만 2026-09-09 확인으로 **출시본은 1.3.0 (177) · 2026-03-23 빌드 · `49f2dc27`** 임이 확정됐다 (백업 데이터의 최종 갱신일 2026-03-23 과도 일치). [데이터 호환성 결정](./data-compatibility-decision.md) §3 |
