...
> 상태 (rev.8): **D9 H 회전 표시 결함 원인 분리 완료, 정식 수정 미적용.** Debug 비교 모드의 회전 왕복만 통과했습니다.
> 최신 [원인·실험·구현안](./single-canvas-rotation-display-investigation.md)과 [실기기 CLI 조작 절차](./device-debugging-cli.md)를 먼저 확인합니다. 기존 D1~D9 실행 이력과 미수행 항목은 아래 기록을 유지합니다.
> **★ D9 (단일 Canvas 실기기 검증) 가 신설됐습니다 — §6-9 · 기록은 §8-7.** 설계 §13 의 마지막 게이트이며,
> 통과해야 flag 기본 활성화와 Phase 4 로 갑니다. 나머지 항목보다 **우선**입니다 (§4-0).
> · 대상 문서: [single-canvas-design.md](./single-canvas-design.md) §11 / §13 / §16 / §18 / §19
> 대상 기기: **iPad mini (A17 Pro)**, **iPad Air (M2)** — 둘 다 60Hz · 8GB (§13 사각지대)
> **★ rev.3 에서 실제 실기기 실행 결과가 들어왔습니다** — 실행 환경은 **iPad mini (A17 Pro) / iOS 27.0 beta
> (24A5408d) / Xcode 26.3 / USB 연결**. 결과는 §8 기록표, 설계 반영은 설계 §11 "D1/D2 실행 결과" ·
> **설계 §18-3-a** · **설계 §20-6**.
> ⚠️ **iPad Air (M2) 는 여전히 미수행입니다.** 아래 모든 결과는 mini 한 대의 것입니다.
> rev.1~rev.2 를 만들 때는 **소스·프로젝트·signing 을 변경하지 않았고, 빌드·테스트도 실행하지 않았으며,
> 백업을 뜨거나 기기를 연결하지도 않았습니다.** rev.3 은 그 문장이 더 이상 사실이 아닙니다 —
> **다만 rev.3 도 소스는 한 줄도 바꾸지 않았습니다.** 저장소에 이미 있는 Debug 빌드와 S4 하네스를 그대로 썼습니다.
...
실제 화면은 사용자 확인으로 판정했고 CLI는 데이터·기하·세대와 회전 제어에 사용했습니다. 상세 결과는 [D9 H 분석](./single-canvas-rotation-display-investigation.md), 재현 명령은 [실기기 CLI 조작](./device-debugging-cli.md)에 있습니다. 회귀 기준선은 **296개**입니다. 아래 rev.1~7 및 이전 기록표의 숫자는 당시 이력이며 현재 기준선으로 사용하지 않습니다.

### rev.7 — D9-3-4 오독 정정 · 합성 프로브 · R18 ★

⚠️ **아래 rev.7 의 E-4 판정은 rev.8 에서 되돌렸습니다** (§8-7 참조) — D9 H 와 같은 자극이라 구분되지 않고, "v1 이라서" 는 세로 재배치를 설명하지 못합니다.

`E-4`(폰트·행간 변경 시 잉크 재배치)를 **결함으로 오독**했습니다. 원인은 그 절들이 **N-Canvas 가 만든
v1 행**이어서 band reflow 대신 평행이동만 된 것이고(§9-3), 단일 Canvas 로 편집하니 v3 로 승격돼
정상 재배치됐습니다(§10-3). **결함이 아닙니다.**

진단이 세 번 헛돈 이유가 계측에 있었습니다 — HUD 의 `sig` 는 **측정** 레이아웃이지 캔버스가 합성에 쓴
레이아웃이 아니어서 둘이 갈라진 상태를 볼 수 없었고, legacy 절이 **개수로만** 나와 겨냥할 수 없었습니다.
**HUD 에 `compose` 줄을 추가했습니다** (`f17f49b5`) — 아래 D9-0-c 와 D9-3 참조.

⚠️ **`leg`/`mism` 은 마지막 합성의 결과입니다.** 획을 그어 v3 로 승격돼도 그 값은 **그 자리에서
갱신되지 않습니다** — 장을 나갔다 와야 재합성됩니다. 이번에 여기서 한 번 더 헛짚었습니다.

### rev.6 — D9 스모크 통과 (R13 · R16 종결) · R16 재확인 절차 신설 ★

스모크에서 나온 결함 2건이 모두 닫혔고 **D9-1(필기) 보류가 풀렸습니다** (설계 §20-13 · §20-14).

⚠️ **R13 수정의 부작용으로 `Δ max` 가 R16 계열을 더 이상 검출하지 못합니다.** 행 높이가 실측 입력이
됐으므로 호스트 제안이 행을 늘려도 레이아웃이 그 렌더를 따라가 Δ 가 0 으로 표시됩니다. 실제로 `.fixedSize`
가 빠진 빌드가 장 전환 Δ 0.00 을 표시하는 것을 이 세션에서 관측했습니다. **§6-9 D9-0-d 를 신설했습니다** —
`H`(totalHeight)를 새 진입값과 비교하는 절차이고, 이제 R16 계열의 유일한 실기기 검출 수단입니다.

### rev.5 — D9 (단일 Canvas 실기기 검증) 신설 ★

설계 §13 이 남긴 **마지막 게이트**를 이 문서에 절차로 넣었습니다. **§6-9** (실행) 과 **§8-7** (기록 양식) 입니다.
rev.4 까지 이 문서는 D1~D8, 즉 **N-Canvas 와 S4 하네스**만 다뤘습니다. D9 는 처음으로 **제품 코드의 단일 Canvas 경로**를
실기기에서 봅니다 — 설계 §8 (편집 계약 · mutation · atomic batch · flush) 이 **테스트로만 검증됐고 실행된 적이 없기** 때문입니다.

⚠️ **rev.3 과 다른 Mac 에서 씁니다.** macOS 26.3 / Xcode 26.3 이 `/Applications/Xcode.app` 에 있고,
`xcode-select` 도 이미 그곳을 가리킵니다 — **AGENTS.md 의 `DEVELOPER_DIR=/Applications/Xcode-26.3.0.app/...` 우회는
그 머신(macOS 27 beta) 전용이며 여기서는 경로 자체가 존재하지 않습니다.** 기기·툴체인 실측값은 §6-9 D9-0.
...
rev.2 의 차단 7건(D1~D6 · D7-pre)은 **전부 같은 사유**였습니다 — 개발 빌드를 기기에 얹을 수 없다는 것.
**멤버십이 활성화되면서 그 사유가 사라졌고**, §3-1-a 가 "재결제 전에 실제로 서명을 시도해 보는 것이
이 판정을 검증하는 가장 싼 방법" 이라고 적은 그대로 **설치가 실제로 성공**했습니다.
D7 도 V4 스키마가 저장소에 들어오면서(`b68b6101`) "산출물 부재" 차단이 풀렸습니다.

**이번 세션에서 실행한 것: D5 · D1 · D2**

| ID | 결과 |
|---|---|
| **D5** | 실기기 N-Canvas baseline 확보. peak **363.6 MB**, 최종 정지 **109.7 MB**, CPU>90% 샘플 **47/188**. **★ 설계 §18-3 (D) 의 "메모리 미회수" 가 재현되지 않았습니다** |
| **D1** | **§11 기준 7·8 이 A ❌ / B ✅ 로 갈렸습니다.** `A정규화` 를 켜도 A 는 동일하게 실패. **오차 = 바깥 `ScrollView` 의 `contentOffset`** 임이 실측으로 확정 |
| **D2** | 기준 9 ✅ (A·B 모두) · 기준 10 ✅ (한 손가락 그리기 / 두 손가락 스크롤). 남은 것은 **제품 과제** |

→ **설계 문서의 스크롤 구조가 B 로 확정됐습니다** (설계 §12 U4).

**부수 확인 4건** (상세는 설계 §20-6)

| # | 내용 | 이 문서 반영 |
|---|---|---|
| 1 | **개발 서명 설치가 App Store 앱의 데이터 컨테이너를 보존합니다.** `Carve.sqlite` 11.1MB 가 그대로 있었습니다 | §2-3 1번 · §10-3 1번 — **해소** |
| 2 | **V4 마이그레이션이 실기기에서, CloudKit entitlement 활성 상태로 성공.** prod DB 는 변동 없음 | §3-1 D7 · §10-2 |
| 3 | **Instruments 기록에는 USB 연결이 필수.** Wi-Fi 는 1.1초 만에 끊기고 `Deferred` 모드라 데이터도 유실 | **§7-3-a 신설** |
| 4 | **`Allocations` 는 오버헤드가 커서 스크롤 측정에 쓸 수 없음.** `Activity Monitor` 사용 | **§7-3-a 신설** |

**무엇을 바꿨나**

| 절 | 변경 |
|---|---|
| 머리말 | 상태를 "차단 해제" 로. 실행 환경 명시 |
| §2-0 / §2-3 | 설치 위험 경고를 **삭제하지 않고**, "실측 결과 컨테이너 보존 확인" 을 덧붙임 |
| §3-1 | D1·D2·D5 → ✅ **완료**. D3·D4·D6·D7·D7-pre → **지금 가능** |
| §4-1 | 지금 실행하는 것을 D3·D4·D6·D7 로 교체. §4-2 의 이월 사유는 해소로 표기 |
| §7 | **§7-3-a 신설** — USB 필수 · Activity Monitor 사용 근거 (재현 조건) |
| §8 | **D5·D1·D2 기록표를 실제로 채움.** 채우지 못한 칸은 빈칸이 아니라 **미측정 사유**를 적음 |
| §10 | 해소된 미확인 항목 갱신 (C1 · 10-3 의 1번) |

**바뀌지 않은 것**

- **§6-1 (D8 백업 추출 경로)** — rev.2 그대로 유효합니다. D8 은 rev.10 에 이미 완료됐습니다.
- **§7-1 · §7-2 (§18-3 과 직접 비교 금지)** — **오히려 더 중요해졌습니다.** 이제 비교할 실기기 수치가 실제로 생겼기 때문입니다.
- **§2-4 백업 절차** — 컨테이너 보존이 관측됐다고 해서 **백업을 건너뛰지 마십시오.** 관측 1건이 정책을 대체하지 않습니다.
...
| 🗄️ **소진된 계획 (보존)** | *(rev.3 신설)* 그 시점의 판단 근거로는 옳았으나 **이미 실행됐거나 전제가 바뀌어 더 이상 따를 순서가 아닌** 절. 삭제하지 않고 남겨 둔 것 |
| ⚠️ | 되돌릴 수 없는 손실 가능성 |

> 🚫 가 붙은 절은 **읽지 말라는 뜻이 아닙니다.** 재결제 후 그대로 실행할 절차이므로 내용을 보존했습니다.
> 다만 **지금 그 명령을 치면 서명 단계에서 실패**합니다 (§0-A ①).
>
> ✅ **rev.3 — 🚫 의 재개 조건은 충족됐습니다.** 이제 그 절들은 **그대로 실행 가능**합니다 (§3-1).
> 표기는 rev.2 의 기록이므로 지우지 않았습니다.
...
> ⚠️ **rev.3 정정 — 위 "27.0 beta 하나뿐" 은 더 이상 사실이 아닙니다.**
> **실기기 세션은 Xcode 26.3 으로 수행**됐습니다 (§8-1). 그 사이에 26.3 이 설치된 것으로 보이나
> **언제 어떻게인지는 확인하지 않았습니다** (§10-2 C3). §1-2 표는 그 시점의 실측 기록이므로 갱신하지 않고 주석만 답니다.
>
> **이것이 D5 판단에 주는 영향:** 툴체인이 §18-3 과 **같아졌습니다**(둘 다 26.3).
> ⚠️ **그렇다고 §7-2 의 "직접 비교 금지" 가 풀리지 않습니다.** 툴체인은 §7-2 의 여섯 가지 이유 중
> **하나(6번)** 일 뿐이고, 나머지 다섯(메모리 회계 · CPU · 렌더 파이프라인 · 메모리 압력 · 입력 방식)은
> **시뮬레이터와 실기기의 차이라서 툴체인과 무관하게 남습니다.**
> 덧붙여 실기기는 **iOS 27.0 beta** 이고 §18-3 은 **시뮬레이터 iOS 26.2** 라, OS 쪽 차이가 새로 생겼습니다.
>
> 설계 §19-4-2 의 경고("TCA 1.20.2 는 Swift 6.3.3 이상에서 컴파일되지 않는다 → Xcode 26.3 필수")도
> **26.3 사용과 정합합니다.**
...
> ### ★ rev.3 — 설치를 실제로 재개했고, **가장 큰 미확인 위험이 해소됐습니다**
>
> 위 문단이 예고한 대로 설치가 재개됐습니다. 그리고 **§2-3 1번의 답이 나왔습니다.**
>
> | rev.2 의 질문 | **rev.3 실측** |
> |---|---|
> | 개발 서명 빌드를 App Store 서명 앱 위에 설치할 때 iOS 가 기존 데이터 컨테이너를 **보존하는가, 삭제하는가** | ✅ **보존합니다.** 설치 후 실사용 파일 `Carve.sqlite` **11.1 MB (3/23/26)** 가 그대로 있었습니다 |
> | Debug 빌드가 prod 데이터를 건드리는가 | ✅ **건드리지 않습니다.** V4 마이그레이션으로 `Carve.dev.sqlite` 는 7.4MB → 8.8MB 로 갱신됐지만 **`Carve.sqlite` 는 변동 없음** — §2-2 1·2번의 코드상 주장이 **실기기에서 실증**됐습니다 |
>
> ⚠️ **그렇다고 §2-4 백업을 건너뛰지 마십시오.**
> **1회 관측**이고, **이 기기·이 iOS 버전(27.0 beta)** 에서의 결과입니다.
> iOS 버전·서명 조합이 바뀌면 동작이 달라질 수 있고, **틀렸을 때 잃는 것은 되돌릴 수 없는 실사용 필사 데이터**입니다.
> **관측 1건이 정책을 대체하지 않습니다.** §2-3 3번(prod CloudKit 에 전부 올라가 있는가)은 **여전히 미확인**입니다.
...
| # | 불확실한 것 | 왜 확인 못 했나 | **rev.3** |
|---|---|---|---|
| 1 | **개발 서명 빌드를 App Store 서명 앱 위에 설치할 때 iOS 가 기존 데이터 컨테이너를 보존하는가, 삭제하는가** | ❓ **미확인.** 기기가 없어 실증 불가. 서명 주체가 달라질 때 iOS 가 앱을 먼저 제거하고 설치하는 동작이 보고돼 있으나, 이 조합에서 어떻게 되는지 단정할 근거가 없습니다 | ✅ **해소 — 보존합니다.** 설치 후 `Carve.sqlite` **11.1 MB (3/23/26)** 가 그대로 있었습니다. iPad mini / iOS 27.0 beta, **1회 관측** |
| 2 | ~~설치 후에도 컨테이너 안에 `Carve.sqlite` 가 남아 있는가~~ | **rev.2 에서 D8 의 전제 조건이 아니게 됐습니다.** D8 은 이제 설치를 하지 않으므로 이 항목에 의존하지 않습니다. D1~D6 재개 시에만 다시 문제가 됩니다 | ✅ **해소** — 1번과 같은 관측입니다 |
| 3 | prod CloudKit 컨테이너에 **모든** 필사 기록이 실제로 올라가 있는가 | ❓ 미확인. 동기화 실패·미로그인·용량 초과 이력이 있었다면 로컬에만 있는 데이터가 존재할 수 있습니다 | ❓ **여전히 미확인.** 확인 수단이 없습니다 |

> ⚠️ **1번이 "삭제" 쪽이면 로컬 실사용 필사 데이터가 사라집니다.**
> prod CloudKit 에 올라가 있지 않은 부분은 **되돌릴 수 없습니다.**
> **재결제 후 설치를 재개할 때는, 그 직전에 §2-4 백업을 반드시 다시 뜨십시오.**

> ✅ **rev.3 — 1번은 "보존" 쪽으로 판명됐습니다.** 위 경고의 **전제**가 해소된 것입니다.
> **그러나 결론(백업을 먼저 뜨라)은 유지하십시오.** 근거는 3번이 여전히 미확인이기 때문입니다 —
> **prod CloudKit 이 안전망이라는 보장이 없는 한, 로컬 백업이 유일한 안전망입니다.**
> 경고를 지우지 않고 결과만 덧붙인 이유입니다.
...
| ID | 항목 | rev.1 판정 | rev.2 판정 | **rev.3 판정** | 한 줄 근거 |
|---|---|---|---|---|---|
| **D1** | Pencil hover offset / live stroke pencil-up 스냅 | 부분 가능 | 🚫 차단됨 (설치 불가) | ✅ **완료** | **A ❌ / B ✅ → 스크롤 구조 B 확정.** 결과는 §8-3, 설계 §11 "D1/D2 실행 결과" |
| **D2** | `.pencilOnly` 손가락 스크롤 / `anyInput` 스크롤 UX | 부분 가능 | 🚫 차단됨 (설치 불가) | ✅ **완료** | 기준 9 ✅ (A·B) · 기준 10 ✅ (두 손가락 스크롤). 남은 것은 **제품 과제** |
| **D3** | 실제 Pencil 지우개로 S1 결과 재확인 | 지금 가능 | 🚫 차단됨 (설치 불가) | ✅ **지금 가능** | 설치가 실제로 성공했으므로 차단 사유가 사라졌습니다. **미수행** |
| **D4** | Pencil 더블탭(지우개) / 두 손가락 더블탭(undo) | 지금 가능 | 🚫 차단됨 (설치 불가) | ✅ **지금 가능** | 위와 같음. **미수행** |
| **D5** | 시편 119편 layout 시간·peak memory·프레임 드랍 | 부분 가능 | 🚫 차단됨 (설치 불가) | ✅ **완료 (부분)** | memory·CPU baseline 확보 (§8-4, 설계 §18-3-a). ⚠️ **layout 시간·hitch 는 미측정** — 아래 |
| **D6** | Stage Manager / 외부 디스플레이 (Air M2) | 지금 가능 | 🚫 차단됨 (설치 불가) | ✅ **지금 가능** | 차단 해제. **미수행.** ⚠️ Air (M2) 는 이번 세션에서 아예 쓰지 않았습니다 |
| **D7** | Phase 1 V4 스키마의 CloudKit 제약 검증 | 차단됨 | 차단됨 (산출물 부재) | ✅ **지금 가능** | **V4 가 저장소에 들어왔습니다** (`b68b6101`, 설계 §20-5). 산출물 부재 차단 해제. **마이그레이션 성공까지는 부수 확인됨** — 아래 |
| **D7-pre** | 실기기에서 dev CloudKit 미러링이 동작하는가 | 지금 가능 | 🚫 차단됨 (설치 불가) | ✅ **지금 가능** | entitlement 가 채워진 개발 서명으로 설치가 성공했습니다. **미러링 관측 자체는 미수행** |
| **D8** | 진짜 legacy `lineData` 1회 추출 (S5 fixture) | 지금 가능 ⚠️ | ✅ 지금 가능 | ✅ **완료** | rev.10 에 완료 (설계 §20-3). 실사용 blob 5건 확보 |

> ★ **rev.2 의 차단 8건이 전부 풀렸습니다.**
> rev.2 는 *"결제 하나로 8개 중 7개가 풀립니다"* 라고 적었고, **그대로 됐습니다.**
> 남은 하나였던 D7(산출물 부재)은 **Phase 1 이 V4 를 구현하면서** 별도로 풀렸습니다.
> §3-1-a 가 "재결제 전에 실제로 서명을 시도해 보는 것이 가장 싼 검증" 이라 적은 그 전제(C1)도
> **설치 성공으로 실증됐습니다** (§10-2).

> ⚠️ **"완료" 의 범위를 정확히 읽으십시오.**
>
> | ID | 완료된 것 | **완료되지 않은 것** |
> |---|---|---|
> | **D5** | 메모리 footprint · CPU 점유 baseline (1회, mini) | **정밀 layout 시간**(`os_signpost` 부재 → Phase 2 이월) · **hitch time ratio**(Activity Monitor 에 지표 없음) · **D5-5 mini vs Air** |
> | **D1** | 기준 7·8 의 A/B 판정 (정성 관측) | 어긋남의 **pt 단위 정량화**. §11 기준 2 의 **관성 fling**, 기준 5 의 **탭/롱프레스** |
> | **D2** | 기준 9·10 판정 | 기준 10 의 **발견 가능성**(제품 과제로 이관) · **palm rejection**(D2-4) |
> | **D7** | V4 마이그레이션이 **실기기에서, entitlement 활성 상태로 성공** (부수 확인) | **미러링 검증 전체.** 설계 §20-5 의 미검증 4건은 그대로 |
...
> ✅ **rev.3 — 이 절 전체가 과거형이 됐습니다.**
> 멤버십이 활성화된 상태로 **개발 서명 설치가 실제로 성공**했고, 설치된 앱이
> **CloudKit entitlement 가 활성인 상태로 동작**했습니다(V4 마이그레이션 성공, 설계 §20-6 ②).
> 즉 **유료 멤버십에서는 이 entitlement 조합이 서명된다는 것이 실증**됐습니다.
>
> ⚠️ **다만 3번 자체(무료 Personal Team 에서 거부되는가)는 여전히 실증되지 않았습니다.**
> 무료 팀으로 시도해 본 적이 없기 때문입니다. **차단이 풀린 경로로 확인했을 뿐, 가설을 검증한 것은 아닙니다** (§10-2 C1).
...
> ✅ **rev.3 — 🚫 표시는 전부 무효입니다.** 차단이 해제됐습니다 (§3-1 rev.3 열).
> **아래 소제목과 🚫 주석은 rev.1/rev.2 시점의 기록으로 보존**하며, 본문의 저장소 근거는 그대로 유효합니다.
> **D1 · D2 · D5 는 이미 수행됐고 결과가 §8 에 있습니다.**
> 남은 **D3 · D4 · D6 · D7 · D7-pre 는 아래 절차를 그대로 실행하면 됩니다.**
...
> **아래 §4-1(D8) 과 §4-2(재결제 이월)는 rev.2 시점의 계획입니다. 둘 다 소진됐습니다.**
> D8 은 rev.10 에 완료됐고, §4-2 의 이월 항목 중 **D5 · D1 · D2 가 rev.3 에서 실행**됐습니다.
> 두 절은 **당시 판단 근거로 보존**하며, 지금 따라야 할 순서는 이 표입니다.

**완료**

| 항목 | 결과 |
|---|---|
| P0 준비 (백업 → 서명 → 빌드 → 설치) | ✅ 설치 성공. **컨테이너 보존 확인** (§2-3 1번 해소) |
| **D5** | ✅ baseline 확보 (§8-4) → **설계 Phase 2 착수 조건 해소** |
| **D1 · D2** | ✅ **스크롤 구조 B 확정** (§8-3, 설계 §12 U4) |
| D8 | ✅ rev.10 완료 (설계 §20-3) |

**남은 것 — 전부 "지금 가능", 권장 순서**

> ★ **rev.5 — 순서가 바뀌었습니다.** 아래 표는 rev.3 시점(Phase 2 착수 전)의 우선순위입니다.
> Phase 3 이 구현되면서 **D9 (§6-9) 가 최우선**이 됐습니다 — 설계 §13 의 **마지막 게이트**이고,
> 단일 Canvas 의 **편집·저장 경로가 아직 한 번도 실행된 적이 없기** 때문입니다.
> **D3 · D4 · D7 은 D9 세션 안에서 상당 부분 함께 처리됩니다** (아래 "묶음" 열).

| 순서 | 항목 | 이걸 하면 풀리는 것 | 묶음 |
|---:|---|---|---|
| **0** | **★ D9 — 단일 Canvas 실기기 검증 (§6-9)** | **flag 기본 활성화·Phase 4 의 선행 조건.** 설계 §8 저장 경로의 첫 실행 · §14 14·16·17 · §11 기준 2·5 · §20-11 롱프레스 | **D3 (지우개) 는 D9-2, D4 (더블탭) 는 D9-3-2, §11 기준 2·5 는 D9-7 로 흡수.** D7 의 dev 스키마 부분은 D9-CK |
| **1** | **D7 나머지** — CloudKit 미러링 (§6-3) | Phase 1 배포 판단. **⚠️ 남은 2건은 기기 2대가 필요**하므로 D9 와 별도 세션 | D9-CK 로 ②③(스키마 승격)만 선행 |
| **2** | D6 — Stage Manager / 리사이즈 (§6-4) | 설계 §11 기준 3 확정 | D9-7-4 에서 단일 Canvas 쪽 1차 관측 |
| **3** | **iPad Air (M2)** — D5-5 · D6 | 기기 간 유의차 (§13 사각지대) | D9 통과 후 |

**rev.3 시점의 순서 (보존)**

| 순서 | 항목 | 이걸 하면 풀리는 것 | 지금 아니면 못 하는가 |
|---:|---|---|---|
| **1** | **D7 · D7-pre** — CloudKit 미러링 (§6-3) | **Phase 1 의 배포 판단.** 설계 §10-3 이 V4 를 forward-only 로 규정하므로 **배포 전에 반드시** 필요합니다. 마이그레이션이 도는 것까지는 확인됐고(설계 §20-6 ②) 남은 것은 미러링입니다 | ⚠️ **Phase 1 배포 전에는 필수** |
| **2** | **D3** — 실제 Pencil 지우개 (§6-5) | 설계 §7-3 승계 전략 / §7-4 `.bitmap` 확정을 **시뮬레이터 결론에서 실기기 결론으로 승격**. 설계 §19-5 의 공백 | — |
| **3** | **D4** — 두 제스처 (§6-6) | 현재 배선의 실동작. undo 결함이 재현되면 설계 §13 의 `SharedUndoManager` 제거 근거가 실증됨 | — |
| **4** | **D6** — Stage Manager / 리사이즈 (§6-4) | **설계 §11 기준 3 이 적용 가능한 기준인지** 결정 → 통과 기준표 확정 | — |

> ★ **D3 · D4 를 할 때 §11 기준 2 의 관성 fling 과 기준 5 의 탭/롱프레스를 함께 처리하십시오.**
> 설계 §20-4 가 "실기기에서 함께 해소된다" 고 적었으나 **rev.3 세션에서는 해소되지 않았습니다** —
> 기준 7~10 에 집중했기 때문이며, 실기기에서 불가능해서가 아닙니다.
> S4 하네스(`-CanvasScrollSpike`)를 띄운 김에 손으로 fling·탭·롱프레스를 해 보면 됩니다.

> ⚠️ **iPad Air (M2) 가 통째로 남아 있습니다.**
> rev.3 의 모든 결과는 **iPad mini (A17 Pro) 한 대**의 것입니다.
> **D5-5(mini vs Air 유의차)** 와 **D6(§13 표가 Air 를 지목)** 이 여기에 걸려 있습니다.

### 4-1. 🗄️ (rev.2 기록) 지금 실행하는 것 — **D8 하나뿐입니다**

> 🗄️ **rev.3: 소진됐습니다.** D8 은 rev.10 에 완료됐습니다. 아래는 당시 판단 근거로 보존합니다.
...
### 4-2. 🗄️ (rev.2 기록) 재결제 이후로 이월 — 순서 자체는 rev.1 그대로 유효

> 🗄️ **rev.3: 재개 조건이 충족됐고 이 순서를 실제로 밟았습니다.**
> 0(P0 준비) → 1(D5) → 6(D1·D2) 이 완료됐습니다. **2~5(D7-pre · D6 · D3 · D4)와 7(D7)이 남았고,
> 지금의 권장 순서는 §4-0 입니다.** 아래는 당시 판단 근거로 보존합니다.
...
> ✅ **rev.3 — 이 리스크(R3)는 실현되지 않았습니다.**
> **Phase 2 착수 전에 D5 를 쟀습니다.** baseline 은 §8-4 이며 설계 §18-3-a 에 반영했습니다.
> 설계 §13 의 순서 `멤버십 활성화 → D5 → Phase 2` 를 지켰습니다.
...
> ✅ **rev.3 — P0-2 ~ P0-7 이 실제로 수행됐고 성공했습니다. 위 표의 🚫 는 전부 무효입니다.**
>
> | 단계 | rev.3 결과 |
> |---|---|
> | P0-3 Signing | ✅ **통과.** 유료 멤버십 활성 상태에서 CloudKit entitlement 를 포함해 서명됨 |
> | P0-5 빌드 | ✅ **성공** — ❓ CLI(`xcodebuild`)인지 Xcode GUI 인지는 **미기록** (§10-3 3번) |
> | P0-6 설치 | ✅ **성공.** ★ **§2-3 1번의 미확인 위험이 여기서 해소됐습니다** — `Carve.sqlite` **11.1 MB 보존** |
> | P0-7 실행 | ✅ **성공.** V4 마이그레이션 완료(`Carve.dev.sqlite` 7.4→8.8 MB), 크래시 없음 |
>
> ⚠️ **P0-6 은 여전히 "되돌릴 수 없는 단계" 입니다.** 이번에 컨테이너가 보존됐다는 것은
> **관측 1건**이지 보장이 아닙니다. **다음 설치 전에도 §2-4 백업을 뜨십시오.**
...
> ✅ **rev.3 — 차단이 전부 해제됐습니다. 아래 🚫 표시는 무효입니다** (§3-1).
>
> | 절 | rev.3 상태 |
> |---|---|
> | §6-1 D8 | ✅ **완료** (rev.10) |
> | §6-2 D5 | ✅ **완료** — ⚠️ **절차는 이 절 그대로 하지 않았습니다.** 실제로 쓴 방법은 **§7-3-a** 와 §8-4 |
> | §6-3 D7-pre / §6-4 D6 / §6-5 D3 / §6-6 D4 | ⏳ **지금 가능. 미수행** — 절차는 **그대로 유효**합니다 |
> | §6-7 D1 / §6-8 D2 | ✅ **완료** — ⚠️ **절차는 이 절 그대로 하지 않았습니다.** **S4 하네스를 실기기에서 돌렸습니다** (§8-3) |
...
> ✅ **rev.3 — 실행 완료. 다만 아래 절차 그대로 하지 않았습니다.**
> 실제로는 **Debug Navigator 시점별 읽기(아래 (1))가 아니라 `xctrace` 타임라인 1회 기록**을 썼고,
> **(2) 의 `Allocations` 는 쓸 수 없었습니다**(오버헤드로 앱이 멈춤).
> **실제로 쓴 방법과 그 근거는 §7-3-a, 결과는 §8-4 입니다. 재측정 시 §7-3-a 를 먼저 읽으십시오.**
> 아래 절차는 rev.2 시점의 계획으로 보존합니다 — (3) hitch / (4) Time Profiler / (5) layout 시간은
> **여전히 미수행**이라 그대로 유효한 지시입니다.
...
> ✅ **rev.3 — 실행 완료. 다만 이 절의 형태(N-Canvas baseline)로 하지 않았습니다.**
> 설계 §13 의 지시대로 **S4 하네스(`-CanvasScrollSpike`)를 실기기에서 모드 A / B 로 띄워**
> **§11 기준 7·8 을 직접 판정**했습니다. 아래 "판정은 아닙니다" 라는 제약은 **S4 하네스가 구현되면서 사라졌습니다.**
> **결과는 §8-3, 결론은 설계 §11 "D1/D2 실행 결과" 와 §12 U4 (B 확정) 입니다.**
...
> ✅ **rev.3 — 실행 완료.** S4 하네스로 수행했습니다. 결과는 §8-3.
> ⚠️ **아래 "§11 기준 9 는 여기서 판정하지 않습니다 — 옵션 B 구조가 없기 때문" 은 무효입니다.**
> S4 하네스가 옵션 B 구조를 제공하므로 **기준 9 도 판정했고, A·B 모두 통과**였습니다.
> **D2-4 palm rejection 은 여전히 미수행**입니다.
...
### 6-9. D9 — 단일 Canvas 실기기 검증 ★ (rev.5 신설)

> **rev.8 추가:** D9 H 회전 표시 결함은 [분석 문서 §5·§6](./single-canvas-rotation-display-investigation.md)의 정식 수정 및 완료 기준으로 판정합니다. CLI 회전과 Debug A/B 실험은 [조작 절차](./device-debugging-cli.md)를 따릅니다. 비교 모드 통과를 기본 경로 통과로 기록하지 않습니다.

> **판정 분류: 지금 가능.** 설치·서명 차단은 rev.3 에서 해소됐고, 검증 대상(Phase 3 산출물)은 저장소에 있습니다.
> **이것이 설계 §13 의 마지막 게이트입니다** — 통과해야 flag 기본 활성화와 Phase 4(구 구조 제거)로 갑니다.
...
| 회귀 기준선 | **265 통과** (Swift Testing 263 + XCTest 2) | ✅ 설계 §19-4-2 유지 |
...
> ✅ **rev.5 — 빌드는 이 머신에서 실제로 통과했습니다.** `** BUILD SUCCEEDED **`,
> 산출물 `~/carve-build/device/Build/Products/Debug-iphoneos/CarveApp.app`,
> 서명 `Identifier=kr.co.carve.leetaek` / `TeamIdentifier=H4MSW7FUBB`.
> **설치·실행은 아직 하지 않았습니다** (🔌) — 아래 두 명령이 D9 세션의 첫 비가역 단계입니다.
...
> ⚠️ **reflow(D9-3-4)를 보기 전에 HUD 의 `leg` 가 0 인지 먼저 확인하십시오** (rev.7).
...
| D9-3-4 | 헤더에서 **폰트·자간·폭 변경**으로 reflow 유발 | 필사가 밑줄 기준으로 재배치 (G4) · ⚠️ **undo 스택이 비워짐** — 설계 §14 **14** (테스트로 고정 안 됨) |
...
> ✅ **rev.3 — D5 를 실제로 측정했습니다.** 결과는 §8-4, 설계 §18-3-a.
> **§7-1 · §7-2 는 그대로 유효하며, §7-2 는 오히려 더 중요해졌습니다** — 이제 나란히 놓고 비교하고 싶어지는
> 실기기 수치가 실제로 존재하기 때문입니다. **§18-3 표와 나란히 놓지 마십시오.**
> 실행하면서 드러난 **재현 조건 2건**은 **§7-3-a** 에 새로 적었습니다. 실행 전에 반드시 읽으십시오.
> ⚠️ **§7-3 의 도구 표는 실행 결과와 어긋난 부분이 있습니다** — §7-3-a 가 정정합니다.
...
> ★ **rev.3 — 아래는 D5 · D1 · D2 를 실행한 세션의 기록입니다.**
...
| ~~🚫~~ **설치한 Debug 빌드 configuration / 번들 ID** | **Debug / `kr.co.carve.leetaek`** — *(rev.2 의 "설치하지 않음" 은 rev.3 에서 무효)* |
| ~~🚫~~ **설치 후 기존 데이터 컨테이너 보존 여부** ★ | ✅ **보존됨.** `Carve.sqlite` **11.1 MB (3/23/26)** 가 그대로 있었습니다 → **§2-3 1번 해소** |
| **★ V4 마이그레이션 결과** *(rev.3 추가)* | ✅ 성공. `Carve.dev.sqlite` **7.4 MB → 8.8 MB**, 크래시 없음. `Carve.sqlite`(prod) **변동 없음** |

> ★ = rev.2 에서 추가된 항목 (백업 추출 경로) 또는 rev.3 에서 추가된 항목 (표기 참조).

### 8-2. D1~D8 판정 요약 (실행 후 갱신)

| ID | rev.2 사전 판정 | 실제 실행 여부 | 결과 (통과 / 실패 / 보류 / 미실행) | 비고 |
|---|---|---|---|---|
| **D1** | 🚫 차단 (설치 불가) | ✅ **실행** | ✅ **통과 (B) / ❌ 실패 (A)** | **§11 기준 7·8 이 A ❌ / B ✅ 로 갈렸습니다 → 스크롤 구조 B 확정** (설계 §12 U4). 상세 §8-3 |
| **D2** | 🚫 차단 (설치 불가) | ✅ **실행** | ✅ **통과** | 기준 9 ✅ (A·B 모두) · 기준 10 ✅. 남은 것은 "두 손가락" 의 발견 가능성 = **제품 과제** |
| D3 | 🚫 차단 (설치 불가) | ❌ **미실행** | **미실행** | 차단 해제됨. **지금 가능** (§4-0) |
| D4 | 🚫 차단 (설치 불가) | ❌ **미실행** | **미실행** | 차단 해제됨. **지금 가능** (§4-0) |
| **D5** | 🚫 차단 (설치 불가) | ✅ **실행** | ✅ **부분 통과** | baseline 확보 → **Phase 2 착수 조건 해소.** ⚠️ layout 시간·hitch·기기 B 는 **미측정**. 상세 §8-4 |
| D6 | 🚫 차단 (설치 불가) | ❌ **미실행** | **미실행** | 차단 해제됨. **지금 가능.** ⚠️ §13 표가 지목한 **Air (M2)** 를 이번 세션에 쓰지 않았습니다 |
| D7 | 차단됨 (산출물 부재) | ⚠️ **부수 확인만** | **보류** | **V4 가 저장소에 들어와 차단 해제** (`b68b6101`). 마이그레이션이 **실기기·entitlement 활성 상태로 성공**했으나 **미러링은 미검증**. 상세 §8-3 |
| D7-pre | 🚫 차단 (설치 불가) | ❌ **미실행** | **미실행** | 설치·entitlement 조건은 충족됐습니다. **import/export 이벤트 관측은 하지 않았습니다** |
| **D8** | ✅ **지금 가능** (백업 경로) | ✅ **실행** | ✅ **통과** | **rev.10 에 완료** (설계 §20-3). 실사용 blob 5건 → S5 fixture |

### 8-3. 항목별 상세 기록

**D8 — legacy `lineData` 추출 (rev.2: 백업 추출 경로)** — ✅ **완료** (`0e9a8449`, 2026-09-05)

> ⚠️ **이 문서(rev.2)는 D8 이 아직 미실행이라고 적었으나, 사실은 이미 실행됐습니다.**
> 런북 rev.2 커밋(`3098680b`)이 **D8 실행 커밋(`0e9a8449`)보다 먼저**라 갱신되지 않은 것입니다.
> **rev.3 에서 바로잡습니다.** 아래 값의 출처는 **설계 §20-3** 입니다.
...
**D1 — Pencil 입력** ★ (rev.3 — **실행 완료. 스크롤 구조가 여기서 결정됐습니다**)

> ⚠️ **rev.1/rev.2 가 D1 을 "N-Canvas baseline" 으로 설계했으나, 실제로는 그렇게 하지 않았습니다.**
...
> **이쪽이 원래 목적이었습니다** — §6-7 이 스스로 *"§11 기준 7·8 의 판정은 아닙니다"* 라고 적었던
> 그 제약이 S4 하네스 덕분에 사라졌습니다.
...
### 8-4. D5 실측 기록 ★ (rev.3 — 실행 완료)

> ⚠️ **rev.1/rev.2 의 (A)~(D) 표 형태로는 기록할 수 없었습니다.**
> 그 표들은 **Debug Navigator 로 시점마다 값을 읽어 적는** 형태를 전제합니다.
> 실제로는 **`xctrace` 로 201초 타임라인 하나를 통째로 기록**했고(§7-3-a ②), 그 안에서 구간을 나눴습니다.
> **원래 표를 지우지 않고 아래에 "실제로 기록된 형태" 를 덧붙입니다** — 두 형태의 차이 자체가 재현 정보입니다.
...
> 위 4개 표(Cold launch / 장 전환 진입 / 장 전체 스크롤 / 메모리 회수)는 **시점별 스냅샷** 형태입니다.
> 이번 측정은 **연속 타임라인**이라 그 칸들에 넣을 값이 대응되지 않습니다.
> **억지로 채우면 지어낸 값이 되므로 비워 두고, 아래 (A′)·(D′) 로 대체합니다.**
> **기기 B(Air) 행은 전부 미측정입니다.**
...
> ★ **rev.3 — "지금 판정 가능?" 열의 "아니오" 는 rev.2 시점 판단입니다.**
> **S4 spike 가 `b53cbfd8` 로 구현되면서 7·8·9 의 전제("spike 필요" / "옵션 B 구조 부재")가 사라졌고**,
> 그 하네스를 실기기에서 그대로 돌려 **네 항목을 모두 판정**했습니다.
...
| **R3** | *(rev.2 선반영)* ⚠️ **D5 baseline 을 Phase 2 착수 전에 못 잴 가능성** | Phase 2 의 `LazyVStack → VStack` 판정 근거가 시뮬레이터 측정만 남음 | ✅ **실현되지 않음 (rev.3)** — Phase 2 착수 전에 쟀습니다. 설계 §18-3-a |
| **R4** | *(rev.3)* ★ **§11 기준 7·8 이 실기기 Pencil 입력에서 A ❌ / B ✅ 로 갈렸다.** `A정규화` 를 켜도 A 는 동일하게 실패 | **스크롤 구조 A 는 채택 불가.** 보정 실패가 아니라 **구조적 모순** — offset 0 고정과 PencilKit 라이브 렌더는 A 에서 양립 불가 | 설계 **§12 U4 (확정: B)** · §11 "D1/D2 실행 결과" · §2 D5 · §17 — **반영 완료** |
| **R5** | *(rev.3)* **오차의 정체 = 바깥 `ScrollView` 의 `contentOffset`** (스크롤 깊이 비례 + 펜슬 그림자는 정상 위치) | `StableCanvasView` 는 **재작성에 흡수** 가 아니라 **접근 자체 폐기**. 스케일 동기화(①)도 A 전용 증상 대응이라 이월 대상이 아님 | 설계 **부록(삭제/격하)** · §13 Phase 4 — **반영 완료** |
| **R6** | *(rev.3)* ★ **설계 §18-3 (D) 의 "메모리 미회수" 가 실기기에서 재현되지 않았다** (363.6MB → 109.7MB 회수) | 설계 §18-5 3번의 면책 논거("기존부터 존재하니 새 설계 탓이 아니다")를 **실기기 평가에 그대로 쓸 수 없음.** Phase 3 이후 실기기에서 회수가 안 되면 **그것은 새로운 현상** | 설계 **§18-3-a** · **§18-5 정정 블록** — **반영 완료** |
| **R7** | *(rev.3)* **개발 서명 설치가 App Store 앱의 데이터 컨테이너를 보존한다** (`Carve.sqlite` 11.1MB 생존). Debug/Release DB 분리도 실증 | §2-3 1번의 최대 미확인 위험이 해소. ⚠️ **다만 1회 관측이므로 §2-4 백업 절차는 유지** | 이 문서 §2-0 · §2-3 · §10-3 — **반영 완료** |
| **R8** | *(rev.3)* **V4 마이그레이션이 실기기에서, CloudKit entitlement 활성 상태로 성공** | **D7 의 일부만 진전.** 설계 §20-5 의 미검증 4건(미러링)은 그대로 → **Phase 1 배포 판단 조건 불변** | 설계 §13 · **§20-6 ②** — **반영 완료** |
| **R9** | *(rev.3)* **Instruments 기록에 USB 필수** (Wi-Fi 는 1.1초 만에 끊기고 `Deferred` 라 데이터도 유실) · **`Allocations` 는 스크롤 측정 불가** | D5 재측정·D6·Phase 2 측정의 **재현 조건** | 이 문서 **§7-3-a** — **반영 완료** |
| **R10** | *(rev.3)* ⚠️ **`allowFingerDrawing == true` 의 "두 손가락 스크롤" 을 사용자가 발견하지 못할 수 있다** | **구조 문제가 아니라 제품 과제.** 안내·온보딩·설정 문구 | 설계 §15 리스크표 — **반영 완료.** 제품 백로그 항목으로 별도 관리 필요 |
| **R11** | *(rev.3)* ⚠️ **iPad Air (M2) 가 통째로 미수행** | **D5-5(기기 간 유의차)** 와 **D6**(설계 §13 표가 Air 를 지목)이 열려 있음. 설계 §13 의 사각지대(120Hz·저메모리)도 그대로 | **실행 판단 필요** — §4-0 |
| **R12** | *(rev.3)* ⚠️ **설계 §20-4 의 "실기기에서 fling·탭/롱프레스도 함께 해소된다" 는 예상이 빗나감** | §11 기준 2·5 가 여전히 미수행. **실기기에서 불가능해서가 아니라 세션이 기준 7~10 에 집중했기 때문** | **D3/D4 수행 시 같은 하네스로 함께 처리** — §4-0 |

### 8-7. D9 실측 기록 (rev.8 — 기존 기록 유지 · H 원인 분리 추가)

**2026-09-08 D9 H 추가 기록:** 기본 경로 회전은 실패, 빈 drawing 경유 복원과 새 획 객체 생성은 정상화, 새 획 자동 적용 Debug 모드의 세로·가로 왕복은 통과했습니다. 판정은 실기기를 본 사용자 응답과 별도 CLI 계측을 결합했습니다. **정식 수정 미적용, H 미종결**입니다. 전체 296개 테스트 통과. [환경·전체 실험표·남은 검증](./single-canvas-rotation-display-investigation.md)을 최신 기록으로 사용합니다. 아래 실행 표의 빈 칸과 이전 기준선은 과거 기록/양식이며 이번 세션의 완료를 뜻하지 않습니다.
...
|---|---|
| 실행일 | |
| 기기 / iOS / 화면(pt) | iPad mini (A17 Pro) `iPad16,2` / iOS 27.0 beta (24A5408d) / |
| Mac / Xcode / configuration | macOS 26.3 (25D125) / Xcode 26.3 (17C529) / Debug (개발 서명) |
| **기기 연결** ★ | **`wired`** (기록 직전 재확인했는가: ) |
| 실행 인자 | `-SingleCanvas -ChapterLayoutOverlay` |
| flag 경로 | 실행 인자 / 설정 토글 (D9-6 은 토글) |
| 백업 (§2-4) | 떴는가: |
| 실기기 빌드 | ✅ rev.5 에서 `BUILD SUCCEEDED` 확인 (설치는 미수행) |
| 회귀 기준선 | 265 (Swift Testing 263 + XCTest 2) |
...
### ★ R16 원인 — `columnHeight` 가 장 전환에 초기화되지 않는다

코드 경로 전부 확인:

| # | 지점 | 사실 |
|---:|---|---|
| 1 | `ChapterCanvasController.columnHeight` | 컨트롤러의 저장 프로퍼티. **컨트롤러는 장 전환에도 살아남고 초기화하는 곳이 없다** |
| 2 | `updateContentGeometry()` | `canvas.contentFrame` 의 height 로 `columnHeight` 를 그대로 쓴다 |
| 3 | `ChapterCanvasView.updateUIViewController` | 컬럼 루트가 `.frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)` — **초과 높이가 컬럼에 제안된다** |
| 4 | `SentencesWithDrawingView.underLineView` | `.frame(maxHeight: .infinity)` 라 세로로 늘어나고 `VStack` 이 초과분을 행들에 나눠 준다 |
| 5 | `setColumnHeight` 의 `guard height != columnHeight` | 늘어난 컬럼을 다시 재도 높이가 같아 아무 일도 안 일어난다 → **안정 고정점** |

```
창세기 1장(31절) → columnHeight ≈ 6460
   ↓ 장 전환 (컨트롤러 유지, columnHeight 그대로)
창세기 2장(이상적 5125.50) 의 호스트 frame = 6460 → 초과분 ≈ 1335
   ↓ underLineView 가 흡수 → 행이 실제로 커짐 → Δ 폭발
   ↓ 다시 재도 6460 → 가드에 걸림
★ 스스로 못 빠져나옴
```
...
| 관측 | 이 모델의 설명 |
|---|---|
| `H`·`sig` 두 경우 동일 | 세로 **분배**만 일어나고 줄바꿈은 그대로 → `textLineCount` 불변 → 레이아웃이 같게 지어진다 |
| `build #1` 고정 | 같은 이유 — 레이아웃 입력이 안 바뀌어 재계산 트리거가 없다 |
| 스크롤로 복구 안 됨 | 5번 가드가 고정점을 잠근다 |
| **N-Canvas 무사** | `ScrollView` 는 무한 높이를 제안 → **초과분 자체가 없다** |
| 새 진입은 깨끗 | `columnHeight == 0` → `contentFrame.height` 가 뷰포트 높이(컬럼보다 작음) → 초과분 없음 |
| `h[min +0.50 max +152.50]` 비균일·비정수 | SwiftUI 가 초과분을 행마다 다르게 분배 |
| 밑줄은 제자리, 빨강 박스만 큼 | 밑줄은 `underlineOffsets` 로 텍스트 위치에 그려지고 행만 늘어난다 |

> `updateContentGeometry` 의 기존 주석이 인접한 함정("content 높이로 늘리면 세로 중앙 배치")을 이미 알고 있었다.
> 중앙 배치는 `alignment: .top` 으로 막았지만 **초과 높이 자체는 남아** `maxHeight: .infinity` 가 흡수한다.

### R16 수정 후 검증 예측 ★ (반드시 4건 모두)

모델이 맞으면 **긴 장 → 짧은 장에서만** 재현된다. 수정 전에 ①②를 확인하면 원인 확정이고, 수정 후에는 4건 모두 깨끗해야 한다.

| # | 전환 | 수정 **전** 예측 | 수정 **후** 기준 | **실측** |
|---:|---|---|---|---|
| ① | 시편 117편(2절) → 창세기 2장 | ✅ 깨끗 **12.00** (이전이 더 짧아 초과분 없음) | 12.00 | ✅ **통과** |
| ② | **시편 119편(176절) → 창세기 2장** | ❌ **1335 보다 훨씬 크게** 재현 (초과분이 큼) | **12.00** | ✅ **통과** |
| ③ | 창세기 2장 → 창세기 1장 (짧은 → 긴) | ✅ 깨끗 **15.00** | 15.00 | ✅ **통과** |
| ④ | 창세기 1장 → 창세기 2장 | ❌ **1335.78** (관측됨) | **12.00** | ✅ **통과** |

> ✅ **R16 종결 — 실기기 4건 전부 통과 기준대로 나왔습니다** (iPad mini A17 Pro · iOS 27.0 beta · USB · `-SingleCanvas -ChapterLayoutOverlay`).
> 전환 후 Δ 가 새 진입 Δ 와 같아졌고, 남은 12.00 · 15.00 은 **R13** 입니다.

②가 가장 강한 판별이다 — 초과분에 비례해 Δ 가 커져야 한다.

> ✅ **`.fixedSize` 가 실제 행의 줄바꿈을 바꾸지 않았습니다** — 오버레이 라벨 `v{절} · {N}줄` 과 실제 밑줄이
> 수정 전후로 같아 보였다는 확인을 받았습니다 (정성 관측). Δ 가 통과 기준으로 나온 것과도 부합합니다.

**⚠️ 수정 후 재확인에서 Δ 말고 하나 더 봐야 한다.** 시뮬레이터 회귀 테스트는 스텁 컬럼으로 **유연성만** 재현한다.
실제 절 행은 `touchIgnoringContextMenu`(중첩 `UIHostingController`)를 거치므로, `.fixedSize` 가 실제 행에서
**본문 줄바꿈이나 밑줄 개수를 바꾸지 않는지**를 눈으로 확인해야 한다 — 오버레이 라벨 `v{절} · {N}줄` 과 실제 밑줄 개수가
수정 전후로 같아야 한다. (코드 추론상 위험은 낮지만 실측은 아니다.)

**한 가지 더.** 전환 순간 `contentSize` 가 한 프레임 이전 장 값으로 남는 과도 구간이 있다.
스크롤 요청은 `layout != nil` 게이트에 막혀 실행되지 않으므로 관측 가능한 영향은 없다고 보지만,
**전환 순간 스크롤바가 튀는지** 볼 가치는 있다.

> ⚠️ 수정 후에도 **12.00 / 15.00 은 남는다.** 그것이 R13(절당 0.5pt)이며 별개 결함이다.
> **R16 수정의 통과 기준은 "Δ 가 0" 이 아니라 "장 전환 Δ == 새 진입 Δ" 다.**
...
→ **G3(절별 귀속)이 장 하단에서 깨집니다.** §2 가 기록한 두 번의 롤백과 같은 계열의 증상이며,
**시뮬레이터 176절 Δ 0.00 에서는 전혀 드러나지 않았습니다.**

> ~~⛔ **원인 확정 전까지 D9-1(필기) 을 시작하지 마십시오.**~~ → ✅ **rev.6 에서 해제됐습니다.**
> R13 이 종결돼 실기기 Δ 가 0.00 입니다 (설계 §20-14). 이제 D9-1 에서 나오는 결함은 레이아웃이 아니라
> 저장 경로입니다.
...
| # | 미해결 |
|---|---|
| 1 | **시뮬레이터(iOS 26.2)에서는 Δ 0.00 이었는데 실기기(iOS 27.0 beta)에서 87.50** — 같은 코드·같은 176절 |
| 2 | 폰트·폭 무관한 **절당 정확히 0.5pt** 의 출처. 0.5pt = @2x 의 1픽셀 |
| 3 | ~~폰트 변경 시 `sig` 미확인~~ → ✅ **해소.** `sig` 가 함께 바뀌었으므로 재계산됐고 실험은 유효 |
| 4 | `h`(heightDelta) 가 **모든 절에서 +0.50 인지**, 아니면 평균만 0.5 인지 — HUD 프로파일 줄로 확인 (rev.5 에서 계측 추가) |
...
| # | 발견 | 영향 | 반영할 곳 |
|---|---|---|---|
| **R13** | ✅ **종결 — 수정 + 실기기 Δ 0.00 (rev.6).** ★ **실기기에서 `ChapterLayout` 예측이 절당 정확히 0.5pt 씩 실측과 어긋나 176절에서 87.5pt 누적.** 방향·폰트·경로(N-Canvas/단일 Canvas) 전부와 무관하고, **장 길이에 선형**(창세기 1장 31절 → 15.00). 0.5pt = @2x 의 1픽셀 | **Phase 3 이 아니라 Phase 2 의 레이아웃 예측 결함.** 허용치 상향은 G3 포기와 같으므로 선택지가 아니었고, **행 높이를 실측 입력으로 승격**해 닫았다 | 설계 §6 · §14 · §15 리스크표 · §16 D9 · **§20-14** |
| **R16** | ✅ **종결 — 수정 + 실기기 4건 통과.** ★ **`ChapterCanvasController.columnHeight` 가 장 전환에 초기화되지 않아, 긴 장 → 짧은 장 전환 시 이전 장의 큰 높이가 텍스트 호스트 frame 이 되고 그 초과분을 행의 `underLineView`(`maxHeight: .infinity`)가 흡수해 행이 실제로 늘어난다.** 늘어난 컬럼을 다시 재도 높이가 같아 `setColumnHeight` 가드에 걸리는 **안정 고정점** — 자가 복구 불가. 창세기 1장 → 2장에서 1335.78pt | **Phase 3 단일 Canvas 전용** (N-Canvas 는 `ScrollView` 가 무한 높이를 제안해 무사). 장 전환은 D9-4 의 핵심 시나리오이고 잉크 좌표계가 깨지므로 **R13 보다 우선.** 기본 활성화 차단 | 설계 **§11 Phase 3 구현표** · §15 리스크표 · §16 D9 |
...
| `gate` | PASS | |
| 실측/기대 절 수 | 176/176 | |
| `Δ max` | 0.00 (tol 1.00pt) | |
| `columnX` | 366.70 | |
| `missing` | 없음 | |
| `first` (첫 레이아웃) | 시뮬레이터 0.71 s | |
| `PKCanvasView` 개수 | 1 | |
...
| 순서 | 항목 | 결과 |
|---:|---|---|
| ② | dev 스키마에 `CD_layoutMetadataData` 생성 | |
| ③ | Development → Production 승격 | |
...
| # | 항목 | 상태 | **rev.3** |
|---|---|---|---|
| C1 | **CloudKit entitlement 가 무료 Personal Team 에서 정말 거부되는가** | ❓ **실증하지 못했습니다.** 인증서가 이미 폐기돼 서명 시도 자체가 불가능합니다. §3-1 의 차단 7건이 **이 전제 하나에 걸려 있습니다.** 재결제 전에 실제로 서명을 한 번 시도해 보는 것이 가장 싼 검증입니다 (§3-1-a) | ⚠️ **여전히 미실증 — 다만 무의미해졌습니다.** 유료 멤버십에서는 이 entitlement 조합이 **서명되고 설치되고 동작한다**는 것이 실증됐습니다(§8-1 · §8-3). **무료 팀으로 시도해 본 적은 없으므로 가설 자체는 검증되지 않았습니다.** 차단이 이미 풀렸으므로 더 이상 판정에 영향을 주지 않습니다 |
| C2 | D6-1/3/6 을 App Store 판으로 관측하는 것이 실제로 되는가 | 🔌 미검증. 앱을 열어 보기만 하면 되므로 위험은 없습니다 (§3-2 D6) | ❓ **여전히 미검증.** D6 자체가 미수행입니다. **다만 이제 개발 빌드로 D6 전체를 할 수 있으므로 이 우회 경로는 불필요해졌습니다** |
| **C3** | *(rev.3 신설)* **이 머신에 Xcode 26.3 이 언제 어떻게 들어왔는가** | — | ⚠️ §1-2 / §1-3 은 *"설치된 Xcode 는 `/Applications/Xcode-beta.app` 하나뿐 = **Xcode 27.0**"* 이라고 실측 기록했는데, **실제 세션은 Xcode 26.3 으로 수행**됐습니다. 그 사이에 26.3 이 설치된 것으로 보이지만 **확인하지 않았습니다.** §1-2 표를 갱신하지 않고 정정 주석만 달았습니다 (§1-3) |

### 10-3. rev.1 부터 남아 있는 항목 (설치 재개 시 다시 유효)

| # | 항목 | 왜 | **rev.3** |
|---|---|---|---|
| 1 | **개발 서명 빌드 설치가 기존 데이터 컨테이너를 보존하는가** | ❓ 실기기 없이는 실증 불가. **rev.2 에서 D8 의 전제 조건은 아니게 됐지만**, D1~D6 재개 시 데이터 안전의 핵심 변수로 남습니다 (§2-3) | ✅ **해소 — 보존합니다.** `Carve.sqlite` **11.1 MB (3/23/26)** 생존. ⚠️ **1회 관측**(iPad mini / iOS 27.0 beta)이므로 **§2-4 백업 절차는 유지**하십시오 |
| 2 | ~~`devicectl device copy from --source` 의 `appDataContainer` 상대 경로 해석~~ | **rev.2 에서 D8 과 무관해졌습니다.** D3 재개 시에만 필요합니다 (§6-5) | ❓ **여전히 미확인.** D3 이 미수행입니다 |
| 3 | `xcodebuild ... -destination 'generic/platform=iOS'` 빌드가 실제로 성공하는가 | 이 문서는 빌드를 실행하지 않았습니다. destination 유효성만 확인 | ⚠️ **빌드·설치는 성공했습니다.** 다만 **이 명령 형태(CLI)로 했는지 Xcode GUI 로 했는지는 미기록**입니다. 명령 자체의 검증으로 삼지 마십시오 |
| 4 | Xcode 27.0 beta 로 이 프로젝트가 문제없이 빌드되는가 | §18 은 Xcode 26.3 기록입니다. 이 머신에는 26.3 이 없습니다 | ❓ **여전히 미확인 — 그리고 질문이 바뀌었습니다.** 세션은 **Xcode 26.3** 으로 수행됐습니다. 즉 §1-2 / §1-3 의 "27.0 beta 하나뿐" 이 더 이상 사실이 아닙니다 (§10-2 C3). **27.0 으로 빌드해 본 적은 없습니다** — 설계 §19-4-2 는 Xcode 27 이 의존성 배포 타깃을 거부한다고 적어 뒀습니다 |
| 5 | `xctrace record --attach` 가 실기기 프로세스에 이름으로 붙는가 (pid 필요한가) | 🔌 연결 후 확인 | ❓ **여전히 미확인.** `--attach` 대신 **`--all-processes`** 를 썼습니다 (§7-3-a) |
| 6 | Stage Manager 에서 세로 고정 + scene manifest 부재 앱이 어떻게 처리되는가 | ❓ D6 이 답할 질문. 일부는 App Store 판으로도 관측 가능 | ❓ **여전히 미확인.** D6 미수행 |
| 7 | prod CloudKit 컨테이너에 실사용 데이터가 **전부** 올라가 있는가 | ❓ 확인 수단 없음. 백업을 필수로 두는 이유 | ❓ **여전히 미확인.** ★ **이것이 §2-4 백업을 계속 필수로 두는 이유입니다** — 1번이 해소돼도 이 항목이 남는 한 로컬 백업이 유일한 안전망입니다 |
| 8 | Apple 이 제시하는 hitch time ratio 절대 임계값 | ❓ 이 문서에서 단정하지 않습니다. A/B 상대 비교로 판정 | ❓ **여전히 미확인.** ⚠️ 더불어 **hitch 자체를 측정하지 못했습니다** — Activity Monitor 에 그 지표가 없습니다 (§7-3-a ②) |
| 9 | ~~백업에서 앱 컨테이너만 추출하는 CLI 경로~~ | ✅ **rev.2 에서 해소.** libimobiledevice 없이 **`/usr/bin/sqlite3` 만으로** 성립합니다 (§6-1). rev.1 이 "경로 없음" 이라고 적은 것은 **틀렸습니다** | ✅ 해소 유지 |
| **10** | *(rev.3 신설)* **D1 어긋남의 pt 단위 크기와 `contentOffset` 과의 정확한 일치** | — | ❓ **정성 관측입니다.** "스크롤 깊이에 비례" 는 **관측된 경향**이며, 회귀식이나 수치 대조를 하지 않았습니다. 결론(구조 A 채택 불가)에는 영향이 없지만, **"오차 = contentOffset" 을 수치로 인용하지는 마십시오** |
| **11** | *(rev.3 신설)* **D5 측정의 스크롤 부하 재현성** | — | ❌ **스크롤 속도·횟수를 계측하지 않았습니다.** 같은 절차를 반복해도 같은 부하가 되리라는 보장이 없습니다 (§8-4 한계 3·4). **Phase 2 전후 비교 시 이 점을 반드시 고려하십시오** |
