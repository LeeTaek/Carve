...
> 상태 (rev.5): **D8 · D5 · D1 · D2 완료. 남은 D3 · D4 · D6 · D7 · D9 는 전부 "지금 가능".**
...
### 8-7. D9 실측 기록 (rev.6 — **스모크 통과, R13 · R16 종결. D9-1 이하 대기**)
...
| **E-4** | ✅ **결함 아님.** 폰트·행간 변경 시 잉크가 재배치되지 않은 절은 **N-Canvas 가 만든 v1 행**이었다. 단일 Canvas 로 편집해 v3 로 승격 후(§10-3) 장 재진입하니 `leg 0` 과 함께 정상 재배치 | 진단에 세 번 헛돌았다. 원인은 계측 부족 — HUD `compose` 줄 추가 (`f17f49b5`) | 런북 §6-9 D9-0-c · D9-3 · 설계 §14 |
