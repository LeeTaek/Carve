...
| **R28** ⚠️ | **혼재 버전 위험 — 구버전 기기가 `drawingVersion == 3` 을 잘못 해석할 수 있다.** 새 버전이 v3 로 저장하면 `drawingVersion` 은 **이미 Production 에 있던 필드라 구버전으로 그대로 동기화**되지만, `layoutMetadataData` 는 구버전 모델에 없어 **무시된다.** 구버전이 `3`(첫 밑줄 기준)의 의미를 모르면 좌표를 다르게 읽어 **필기가 엉뚱한 위치에 그려질 수 있다** | ⚠️ **"모르는 필드를 무시한다" 가 여기서는 안전장치가 아니다** — 표식은 넘어가는데 해석에 필요한 정보는 안 넘어간다. 위험 여부는 **현재 App Store 빌드가 V4(코덱이 3 을 아는 버전)인지**에 달렸다 — **미확인.** 기기 2대를 쓰는 사용자에게만 나타난다. 필요하면 최소 버전 게이트를 함께 검토한다 | 설계 §10-1 · §10-2 |
