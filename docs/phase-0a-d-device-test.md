...
> **상태 (rev.10):** D1~D8 은 전부 완료됐고 (§8-2), 지금 살아 있는 항목은 **D9 — 단일 Canvas 실기기 검증**입니다.
> **절차 §6-9 · 기록 §8-7.** 설계 §13 의 마지막 게이트이며, 통과해야 flag 기본 활성화와 Phase 4 로 갑니다.
>
> **D9 진행 상황 (2026-09-08)** — 스모크 · D9-1 · D9-2 · **D9-3-1·2·4** · **D9-4-1·4** · **D9-7-4** · **D9-8**(측정) · D9-CK② 통과.
> ⛔ **D9-6 은 ❌ 실패로 정정됐습니다** — flag off 로 장을 **열기만 해도** v3 절이 v2 로 강등되고 metadata 가 삭제됩니다 (**R23**, 설계 §10-3 위반). 이전 ✅ 는 화면만 보고 DB 를 안 본 판정이었습니다. **R24**(flag off 직후 화면 밖 절 미표시)도 함께 재현됐습니다. **둘 다 기본 활성화 차단 사유입니다.**
> ✅ **D9 H(회전 표시)와 E-4 는 종결됐습니다** — 분석 §6 의 잔여 검증 5건을 모두 수행했고 D9 H 증상은 전부 통과했습니다. 남은 위험은 **R20**(재구성의 전이 메모리 peak 1.8 GiB) · **R21**(N-Canvas 대조 미측정) · **R17 · R18 · R22** — §8-7.
> **D9-5 · D9-7 나머지 · D9-CK③ 은 ❓ 미수행**입니다.
>
...
> · 회귀 기준선 **302** (설계 §19-4-2). **아래 기록에 남은 265 · 296 등은 그 시점의 이력이며 현재 기준선이 아닙니다.**
...
> ⛔ **D9-6 ❌ 실패** (R23 — flag off 로 열기만 해도 v3→v2 강등 + metadata 삭제) · **D9-5 · D9-7 나머지 · D9-CK③ ❓ 미수행** (§8-7).
>
...
| 회귀 기준선 | **302** (설계 §19-4-2) | ⚠️ 이 표가 작성될 당시(rev.5)는 265 였습니다. **줄어들면 회귀입니다** |
...
> **D5 는 측정했고 결과는 §8-4 · 설계 §18-3-a 입니다. D9-8 은 아직 ❓ 미수행이며 이 절의 방법을 그대로 씁니다.**
...
> | **D9-2** 지우개 | ✅ **통과** |
> | **D9-6** flag 토글과 롤백 | ❌ **실패로 정정 (2026-09-08)** — 표시는 정상이나 **flag off 로 장을 열기만 해도 v3 절이 전부 v2 로 강등되고 metadata 가 삭제된다** (R23). 이전 ✅ 는 화면만 보고 DB 를 안 본 판정이었다 |
...
> | **D9-H** 회전 표시 | ✅ **증상 해소 확인** — 조사 §6 잔여 5건 중 4건 통과, 편집·저장 왕복은 **N-Canvas 왕복만 R23 으로 실패** |
> | **D9-5 · D9-7 나머지 · D9-CK③** | ❓ **미수행** |
>
> ⚠️ **절 단위 판정입니다 — 아래 "판정 요약" 표의 항목별 관측은 기록되지 않았습니다.**
> 재수행할 때 채우십시오.
>
> **살아 있는 결함·미확정:** **E-4**(미확정) · **R17** · **R18** — 이 절 마지막 "후속 작업" 표.
...
| 백업 (§2-4) | ❓ 미기록 |
| 실기기 빌드·설치·실행 | ✅ 성공 |
| 회귀 기준선 | **302** (설계 §19-4-2). ⚠️ 아래 기록에 남은 265 · 296 은 그 시점의 이력입니다 |
...
| D9-5-2 | 올바른 절이 잡힘 | | |
| D9-5-3 | 본문 쪽 x 클램프 | | |
| D9-5-4 | 회차 복원 (mutation 미생성) | | |
| D9-5-5 | 미저장 pending 유실 없음 | | |
| D9-5-6 | ⚠️ 손가락 필사 ON 에서 롱프레스 충돌 | | |
...
| D9-7-1 | **관성 fling** (§11 기준 2) | | |
| D9-7-2 | 깊은 offset 에서 필기 | | |
| D9-7-3 | **탭 / 헤더 애니메이션** (§11 기준 5) | | |
...
| 4 | iPad Air (M2) 미수행 |
| 5 | **D5 와 직접 비교하지 않았다** — D5 는 N-Canvas 이고 이번은 단일 Canvas 두 변형의 A/B 다. 절대값 차이(363.6 MB → 944.8 MiB)의 원인은 이 측정으로 규명되지 않았다 → 후속 **R21** |
...
| ③ | Development → Production 승격 | ❓ **미수행** — ⚠️ 되돌릴 수 없는 외부 상태 변경이라 **사람이 직접** 해야 합니다. **단일 Canvas 기본 활성화 전 필수** (설계 §10-1-a) |
...
| **R20** | ⚠️ **표시용 획 재구성이 회전마다 큰 전이 메모리 peak 을 만든다.** 시편 119편(176절·61획)에서 `memory-physical-footprint` peak 이 기본 **1759.6 / 1817.8 MiB** vs 대조 **995.3 / 1173.6 MiB** (n=2씩, 교차 실행). 안정 상태는 두 경로가 같아 **누수가 아니라 전이**다. 진입만 하는 A/B 에서는 차이가 없다(944.8 vs 945.4 MiB) | 대조가 메모리를 덜 쓰는 이유는 **결함 그 자체**로 보인다 — 캐시 재사용으로 렌더를 건너뛰기 때문. 즉 낭비가 아니라 결함이 생략하던 일이다. **그래도 8 GB 기기에서 1.8 GiB 전이 peak 은 실측이고, 저메모리 iPad 는 미검증이다** | 이 문서 §8-7 D9-8 · §10-3 |
| **R21** | ⚠️ **단일 Canvas 의 시편 119편 진입 peak(944.8 MiB)이 D5 의 N-Canvas baseline(363.6 MB)과 크게 다르다.** 이번 세션은 `-CanvasReuseStrokesOnApply` A/B 만 했고 **N-Canvas 대조를 뜨지 않았다** | 원인 미규명. 측정 조건 차이(iOS 27 beta·다른 세션·다른 스크롤 절차)일 수도, 단일 Canvas 자체의 비용일 수도 있다. **기본 활성화 전에 같은 절차로 N-Canvas 를 한 번 재야 한다** | 이 문서 §8-7 D9-8 한계 5 |
...
| **R23** ★ | ⛔ **flag off 로 장을 열기만 해도 그 장의 v3 절이 전부 v2 로 강등되고 `layoutMetadataData` 가 삭제된다 — 사용자 편집이 없어도.** 2026-09-08 실측: 시편 122편 9개 절이 획을 하나도 긋지 않았는데 `v3`→`v2`, `meta` 217~225 B→**0** (1~8절 15:57:58, 9절 15:58:20). `lineData` 는 불변이라 **획은 잃지 않는다.** **원인:** `CanvasView` 가 `canvas.drawing` 을 프로그램으로 대입할 때([CanvasView.swift:45](../Feature/CarveFeature/Sources/Presentation/Drawing/Canvas/CanvasView.swift:45) · [:60](../Feature/CarveFeature/Sources/Presentation/Drawing/Canvas/CanvasView.swift:60)) PencilKit 이 `canvasViewDrawingDidChange` 를 부르는데, N-Canvas 의 coordinator 에는 **억제 장치가 없어**([CanvasView.swift:88](../Feature/CarveFeature/Sources/Presentation/Drawing/Canvas/CanvasView.swift:88)) 곧바로 `.saveDrawing` 이 나가고 그 안에서 강등이 일어난다([CanvasFeature.swift:76](../Feature/CarveFeature/Sources/Presentation/Drawing/Canvas/CanvasFeature.swift:76)). 단일 Canvas 는 같은 자리를 `isApplyingDrawing` 으로 막는다([ChapterCanvasController.swift:408](../Feature/CarveFeature/Sources/Presentation/Drawing/ChapterCanvas/ChapterCanvasController.swift:408)) | ⛔ **설계 §10-3 위반.** 설계는 "v3 는 N-Canvas 에서 **표시**는 `displayTransform` 무변환, **편집할 때만** v2 로 내리고 metadata 를 지운다" 이고 런북 D9-6-4 도 "그 절을 **편집**" 이다. **저장소가 forward-only 라 flag off 가 유일한 롤백 수단인데, 그 롤백 경로 자체가 데이터를 조용히 열화시킨다.** metadata 를 잃은 절은 band reflow 를 못 하고(설계 §9-3) 단일 Canvas 로 **다시 편집한 절만** v3 로 복구된다. ⚠️ **기본 활성화의 차단 사유.** D9 H(`ef053111`) 와는 무관한 선재 결함이며, v3 행이 존재해야 도달하므로 지금까지 드러나지 않았다 | 설계 **§10-3 · §14 16** · 이 문서 §6-9 D9-6 |
| **R24** | ⚠️ **flag off 직후 화면 밖이었던 절의 잉크가 표시되지 않는다.** 시편 122편 9절이 토글 off 직후 비어 보였고, **스크롤로 왕복해도 살아나지 않다가 회전(레이아웃 재계산) 후에야 나타났다** (2회 관측, 재현됨). 9절의 강등 쓰기 시각(15:58:20)이 나머지 절(15:57:58)보다 22초 늦은 것과 맞물린다 | R23 과 같은 뿌리로 보인다 — 그 절의 N-Canvas 행이 레이아웃 재계산 전까지 만들어지지 않는다. **데이터는 살아 있어 ⛔ 중단 기준("flag off 에서 데이터가 안 보임")에는 걸리지 않으나, 사용자에게는 필기가 사라진 것으로 보인다** | 설계 §10-3 · 이 문서 §6-9 D9-6 |
