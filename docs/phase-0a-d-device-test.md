...
> 상태: **부분 차단 — 지금 실행 가능한 것은 D8 하나** (rev.2) · 대상 문서: [single-canvas-design.md](./single-canvas-design.md) §11 / §13 / §16 / §18 / §19
> 대상 기기: **iPad mini (A17 Pro)**, **iPad Air (M2)** — 둘 다 60Hz · 8GB (§13 사각지대)
> 이 문서를 만들면서 **소스·프로젝트·signing 을 변경하지 않았고, 빌드·테스트도 실행하지 않았습니다.**
> **백업을 실제로 뜨거나 기기를 연결하지도 않았습니다.**
> 아래 명령은 **이 머신에서 실제로 실행하거나 `--help` 로 조회해 확인한 것**과
> **기기·백업이 있어야만 확인 가능한 것**을 표기로 구분했습니다.

---

## 0-A. 개정 이력

### rev.2 — D8 을 백업 추출 경로로 교체, 나머지 실기기 항목 전면 차단

**왜 개정했나 — 전제 두 개가 무너졌습니다**

**① 유료 멤버십 만료 + Apple Development 인증서 폐기 → 기기에 설치할 수 없습니다.**

`App/CarveApp/Support/Carve.entitlements` 가 `com.apple.developer.icloud-container-identifiers` 와
`com.apple.developer.icloud-services` 를 요구합니다. 이 둘은 **유료 Apple Developer Program 전용 entitlement** 라
무료 Personal Team 서명으로는 프로비저닝 프로파일이 만들어지지 않습니다.
entitlement 를 빼는 우회도 불가능합니다 — `SwiftDataContextProvider+Dependency.swift:36` 이
`cloudKitDatabase: .private(containerId.id)` 로 하드코딩돼 있어 **소스 변경이 따라오기 때문**입니다(AGENTS.md 금지 사항).
→ P0-5(빌드) · P0-6(설치) 이후가 전부 막히고, 그에 의존하는 **D1~D6 · D7-pre 가 모두 차단**됩니다.

**② 더 근본적으로, rev.1 의 D8 경로는 애초에 성립하지 않았습니다.**

`devicectl device copy from` 과 Xcode *Download Container…* 는
**개발 서명으로 설치된 앱**의 컨테이너만 접근합니다 — rev.1 §5 P0-6 이 스스로 인용한 도움말 조건
(*Third-party / Installed via Xcode (profile validated) / Not enterprise signed*) 그대로입니다.
그런데 D8 이 꺼내려는 것은 **App Store 로 설치된 앱**의 실사용 데이터입니다.
rev.1 은 "개발 빌드를 덮어 설치하면 기존 컨테이너가 살아남을 것" 이라는 ❓ 미검증 가정(rev.1 §2-3 1번)에
전적으로 기대고 있었고, **그 가정이 참이었더라도 덮어 설치하는 순간 App Store 설치 상태가 파괴**됩니다.
즉 인증서가 멀쩡했더라도 이 경로는 틀렸습니다.

**→ 실제로 성립하는 유일한 경로는 Finder 로컬 백업에서 추출하는 것입니다.**
백업은 앱을 건드리지 않고, App Store 설치 상태를 그대로 둔 채, 실사용 컨테이너의 사본을 만듭니다.
설치가 필요 없으므로 rev.1 이 D8 앞에 세워 둔 **가장 큰 위험(덮어 설치)이 통째로 사라집니다.**

**무엇을 바꿨나**

| 절 | 변경 |
|---|---|
| §0 | 🚫 표기 신설, 판정 분류에 "차단됨(설치 불가)" 추가 |
| §2 | "Debug 설치가 App Store 판을 덮어쓴다" 경고를 **삭제하지 않고 유예 상태로 재구성**. §2-4 는 "복원용 안전망" 이 아니라 **D8 의 추출 원본** 으로 역할 변경 |
| §3 | D1~D6 · D7-pre → **차단됨(설치 불가)**. D8 → **지금 가능**(백업 경로) |
| §4 | D8 단독 실행. 나머지는 **유료 멤버십 재결제 이후로 이월** |
| §5 | P0-3 / P0-5 / P0-6 / P0-7 에 🚫 + 사유 + 재개 조건. **삭제하지 않음** |
| §6-1 | **전면 재작성** — `devicectl` → Finder 로컬 백업 + `Manifest.db` 추출 |
| §8-1 / §8-3 | D8 기록 항목을 백업 경로 기준으로 교체 |
| §9 | 설치를 안 하므로 원복 절차 대부분이 무효. 백업 경로 기준으로 축소 |
| §10 | 백업 경로에서 새로 생긴 미확인 항목 추가 |

**바뀌지 않은 것**

- §7 (D5 측정 방법론) 과 §8 (기록 양식) 은 **그대로 유효**합니다. 실행 시점만 미뤄졌습니다.
- §1-1 (저장소 상태) · §1-2 (툴 실측) 도 그대로 유효합니다.
- **D8 의 목적과 판정 의도는 동일**합니다. 수단만 바뀌었습니다.

### rev.1 — 최초 작성

`devicectl` 기반 D8 + D1~D7-pre 실기기 런북. 위 ②의 이유로 D8 절이 무효화됐습니다.
...
| 🚫 **차단 (재개 조건 있음)** | *(rev.2 신설)* 지금은 실행할 수 없지만 **명시된 조건이 충족되면 그대로 되살아나는** 절차. 삭제하지 않고 남겨 둔 것 |
| ⚠️ | 되돌릴 수 없는 손실 가능성 |

> 🚫 가 붙은 절은 **읽지 말라는 뜻이 아닙니다.** 재결제 후 그대로 실행할 절차이므로 내용을 보존했습니다.
> 다만 **지금 그 명령을 치면 서명 단계에서 실패**합니다 (§0-A ①).
...
> ⚠️ **rev.2 주의 — `§10` 이 새로 겹쳤습니다.**
> rev.2 에서 이 문서의 §10 이 하위 절(§10-1 ~ §10-3)로 나뉘면서 설계 문서 §10-2 와 번호가 충돌합니다.
> 그래서 **설계 문서 쪽은 전부 `설계 §10-2` 로 명시**했습니다. 접두어 없는 `§10-1` / `§10-2` / `§10-3` 은 **이 문서**입니다.

### 판정 분류 (rev.2 에서 4분류로 확장)

| 분류 | 정의 | 해소 조건 |
|---|---|---|
| **지금 가능** | 코드 변경 없이, **지금 이 머신과 기기 상태에서** 검증까지 끝낼 수 있음 | — |
| **부분 가능** | 현재 구조(N-Canvas)에 대한 baseline·현상 확인만 가능. §11 통과 기준 판정은 S4 spike 구현 후 | S4 spike 구현 |
| **차단됨 (설치 불가)** | *(rev.2 신설)* 검증 대상은 저장소에 있으나, **개발 빌드를 기기에 설치할 수단이 없어** 실행 자체가 불가능 | **유료 Apple Developer Program 재결제 + Apple Development 인증서 재발급** (§0-A ①) |
| **차단됨 (산출물 부재)** | 선행 Phase 산출물이 저장소에 없어서 **검증 대상 자체가 존재하지 않음** | 해당 Phase 구현 |

> rev.1 은 "차단됨" 이 하나뿐이었고 뜻이 **산출물 부재**였습니다.
> rev.2 에서 성격이 전혀 다른 차단(설치 불가)이 생겨 둘을 분리했습니다.
> **해소 조건이 다르므로 같은 이름으로 묶으면 안 됩니다** — 하나는 코드를 짜야 풀리고, 하나는 결제하면 풀립니다.
...
| 1.2.0 실제 배포 | **태그 존재** (`ver1.2.0`, `ver1.2.1`) → 설계 §10-2 의 절대좌표 데이터가 존재할 개연성 | `git tag` |
...
| `idevicebackup2` / `ideviceinfo` / `cfgutil` | **미설치** (libimobiledevice · Apple Configurator 없음) | ✅ `which` 확인 |
| **`sqlite3`** ★ | `/usr/bin/sqlite3` — **3.54.0** (2026-04-09, Apple 빌드). `median()` 집계 함수까지 사용 가능 | ✅ 실행 확인 |
| **`plutil`** ★ | `/usr/bin/plutil`. `-extract <key> raw -o -` 로 바이너리 plist 값을 꺼낼 수 있음 (공백 포함 키도 따옴표로 감싸면 동작) | ✅ 모의 plist 로 실행 확인 |
| `shasum` / `xxd` / `file` / `ditto` / `rsync` | 전부 존재 (`/usr/bin/`) | ✅ `which` 확인 |
| `python3` | `/usr/bin/python3` — 3.9.6 (Command Line Tools 동봉) | ✅ 확인함 |

> ★ **rev.2 에서 추가된 행입니다.** D8 이 백업 추출 경로로 바뀌면서 필요해진 도구들이며,
> **libimobiledevice 나 Apple Configurator 없이 `sqlite3` + 셸만으로 추출이 성립합니다** (§6-1).
> rev.1 이 위 행에 "→ 백업 추출 CLI 경로 없음" 이라고 적었던 것은 **틀렸습니다.**
> 비암호화 백업의 `Manifest.db` 는 **평문 SQLite** 라 `/usr/bin/sqlite3` 로 그대로 열립니다.
...
✅ **링크수 2 ⇒ 항목 0개 ⇒ 이 Mac 에 기존 로컬 백업은 없습니다.**
(백업은 기기 UDID 이름의 하위 디렉터리로 저장됩니다.)

> 이것이 중요한 이유: **오래된 백업이 남아 있으면 새로 뜬 백업 대신 그것을 추출해
> 낡은 데이터를 fixture 로 만들 위험**이 있습니다. 지금은 그 함정이 없으므로,
> §6-1 에서 만드는 백업이 유일한 백업입니다.
> ❓ 다만 TCC 하에서 링크수가 항상 신뢰 가능한지는 별도로 검증하지 않았습니다.
...
### 2-0. rev.2 요약 — 위험은 **사라진 것이 아니라 유예됐습니다** ★

| | rev.1 | rev.2 (지금) |
|---|---|---|
| 주된 위험 | **Debug 빌드 덮어 설치**가 App Store 판의 데이터 컨테이너를 지울 수 있음 | **해당 없음.** 설치 경로 자체가 막혀 있음 (§0-A ①) |
| D8 이 기기에 하는 일 | 설치 → 실행 → 컨테이너 복사 (**쓰기 발생**) | **아무것도 하지 않음.** Finder 백업만 (**읽기**) |
| 백업의 역할 | 설치 실패 시 되돌릴 **안전망** | **추출 원본 그 자체** (§6-1) |
| 되돌릴 수 없는 단계 | P0-6 설치 | **없음** (D8 절차 안에서. §2-4-a 의 "모든 설정 재설정" 처럼 사용자가 따로 선택하는 것은 예외) |

> ⚠️ **아래 §2-1 ~ §2-3 을 삭제하지 않은 이유:**
> 유료 멤버십을 재결제하고 인증서를 재발급받는 순간 **이 경고들이 그대로 다시 유효해집니다.**
> D1~D6 · D7-pre 를 재개하려면 반드시 설치를 해야 하고, 그때 §2-3 의 미확인 위험이 되살아납니다.
> 지금은 "읽되 실행하지 않는" 상태로 두십시오.

### 2-1. 🚫 (유예) 위험의 정체 — Debug 빌드가 App Store 앱과 **같은 번들 ID** 를 씁니다

> **적용 시점:** 유료 멤버십 재결제 후 **설치를 재개하는 순간부터.** 지금은 해당 없음.
...
**이 사실은 rev.2 에서도 그대로 참입니다.** 다만 지금은 설치할 수단이 없어 **발동하지 않을 뿐**입니다.
...
### 2-3. 🚫 (유예) 안전하다고 **확인하지 못한** 것 ⚠️

> **적용 시점:** §2-1 과 동일 — 설치를 재개하는 순간부터. 지금은 발동하지 않습니다.
...
| 2 | ~~설치 후에도 컨테이너 안에 `Carve.sqlite` 가 남아 있는가~~ | **rev.2 에서 D8 의 전제 조건이 아니게 됐습니다.** D8 은 이제 설치를 하지 않으므로 이 항목에 의존하지 않습니다. D1~D6 재개 시에만 다시 문제가 됩니다 |
...
> **재결제 후 설치를 재개할 때는, 그 직전에 §2-4 백업을 반드시 다시 뜨십시오.**

### 2-4. 백업 — rev.2 에서 **역할이 바뀌었습니다** ★

| | rev.1 | rev.2 |
|---|---|---|
| 목적 | 설치가 데이터를 지울 경우의 **복원용 안전망** | **D8 의 추출 원본.** 이것 없이는 D8 자체가 성립하지 않음 |
| 암호화 | **암호화 권장** (키체인까지 포함한 완전 복원) | **비암호화 권장** — 아래 근거 |
| 시점 | 다른 모든 단계보다 먼저 | 여전히 먼저. 단 뒤따르는 위험한 단계가 없음 |

**왜 이번엔 비암호화인가**

| # | 근거 |
|---|---|
| 1 | **"설치 전 안전장치" 라는 원래 목적이 사라졌습니다.** 설치할 것이 없으므로 완전 복원 가능성을 최우선으로 둘 이유가 없습니다 |
...
## 3. D1~D8 실행 가능성 판정 ★ (rev.2 재분류)

### 3-1. 요약표

| ID | 항목 | rev.1 판정 | **rev.2 판정** | 한 줄 근거 |
|---|---|---|---|---|
| **D1** | Pencil hover offset / live stroke pencil-up 스냅 | 부분 가능 | 🚫 **차단됨 (설치 불가)** | 실기기 조작이 필요 → **개발 빌드 설치 불가** (§0-A ①). 구조적 한계(N-Canvas baseline)는 그대로 |
| **D2** | `.pencilOnly` 손가락 스크롤 / `anyInput` 스크롤 UX | 부분 가능 | 🚫 **차단됨 (설치 불가)** | 위와 같음. §11 기준 10 판정도 설치 후에만 가능 |
| **D3** | 실제 Pencil 지우개로 S1 결과 재확인 | 지금 가능 | 🚫 **차단됨 (설치 불가)** | 위와 같음. 코드는 살아 있으나 실행할 빌드를 얹을 수 없음 |
| **D4** | Pencil 더블탭(지우개) / 두 손가락 더블탭(undo) | 지금 가능 | 🚫 **차단됨 (설치 불가)** | 위와 같음 |
| **D5** | 시편 119편 layout 시간·peak memory·프레임 드랍 | 부분 가능 | 🚫 **차단됨 (설치 불가)** | Instruments 는 **개발 서명된 앱**에만 붙습니다. App Store 판에는 프로파일링 권한이 없습니다 |
| **D6** | Stage Manager / 외부 디스플레이 (Air M2) | 지금 가능 | 🚫 **차단됨 (설치 불가)** | ⚠️ **일부는 App Store 판으로도 관측 가능** — 아래 §3-2 참조 |
| **D7** | Phase 1 V4 스키마의 CloudKit 제약 검증 | 차단됨 | **차단됨 (산출물 부재)** | V4 스키마가 저장소에 없음 (V1/V2/V3 만). **이건 결제와 무관합니다** |
| **D7-pre** | 실기기에서 dev CloudKit 미러링이 동작하는가 | 지금 가능 | 🚫 **차단됨 (설치 불가)** | dev 컨테이너에 붙으려면 **CloudKit entitlement 가 채워진 개발 서명**이 필요 — 정확히 그것이 불가능한 이유 |
| **D8** | 진짜 legacy `lineData` 1회 추출 (S5 fixture) | 지금 가능 ⚠️ | ✅ **지금 가능** | **경로가 바뀌어 오히려 안전해졌습니다.** Finder 로컬 백업 → `Manifest.db` → 파일 추출. 기기에 쓰지 않고, 설치도 하지 않음 (§6-1) |

> ★ **결제 하나로 8개 중 7개가 풀립니다.**
> D1~D6 · D7-pre 의 차단 사유는 **전부 같습니다** — 개발 빌드를 기기에 얹을 수 없다는 것 하나입니다.
> 유료 Apple Developer Program 재결제 + Apple Development 인증서 재발급이면 rev.1 의 절차가 **그대로** 되살아납니다.
> 그래서 이 문서는 해당 절차를 지우지 않고 🚫 로 표시만 했습니다.
> **D7 만 성격이 다릅니다** — V4 스키마를 짜야 풀리므로 결제로는 해소되지 않습니다.

### 3-1-a. ⚠️ 차단 판정의 근거 — "entitlement 가 유료 전용" 이라는 부분

| 확인한 것 | 결과 |
|---|---|
| `App/CarveApp/Support/Carve.entitlements` 내용 | ✅ `com.apple.developer.icloud-container-identifiers` (컨테이너 2개) + `com.apple.developer.icloud-services` = `CloudKit`. 이 머신에서 파일을 직접 읽어 확인 |
| entitlement 를 빼는 우회가 코드 변경을 부르는가 | ✅ 그렇습니다. `SwiftDataContextProvider+Dependency.swift:36` 과 `:56` 두 곳이 `cloudKitDatabase: .private(containerId.id)` 로 하드코딩돼 있습니다 |
| **iCloud/CloudKit entitlement 가 무료 Personal Team 에서 정말 거부되는가** | ❓ **이 문서에서 실증하지 못했습니다.** 인증서가 이미 폐기돼 서명 시도 자체를 할 수 없습니다. 이 판정은 **사용자가 전달한 전제**에 근거합니다 |

> 위 3번이 만약 사실과 다르다면(무료 팀으로도 서명이 된다면) D1~D6 · D7-pre 의 차단은 해제됩니다.
> **재결제 전에 한 번 실제로 서명을 시도해 보는 것이 이 판정을 검증하는 가장 싼 방법입니다.**

### 3-2. 항목별 근거 (저장소에서 직접 확인)

> 🚫 **아래 D1~D7-pre 의 소제목에 적힌 "부분 가능 / 지금 가능" 은 rev.1 판정입니다.**
> rev.2 에서는 **전부 "차단됨 (설치 불가)"** 입니다 (§3-1).
> 소제목을 고치지 않고 남긴 이유는, 여기 적힌 내용이 **차단 사유와 무관한 저장소 근거**이기 때문입니다 —
> 즉 **재결제 후에 그대로 다시 쓰는 판정 근거**입니다. "지금 실행하라" 는 뜻으로 읽지 마십시오.
>
> **예외로 D6 만 부분적으로 관측 가능합니다** — 아래 D6 항목의 rev.2 주석을 보십시오.

**D1 — 부분 가능** *(rev.1 판정 · rev.2: 🚫 차단)*
...
**D5 — 부분 가능** *(rev.1 판정 · rev.2: 🚫 차단 — Instruments 는 개발 서명된 앱에만 붙습니다)*
...
**D6 — 지금 가능** *(rev.1 판정 · rev.2: 🚫 대체로 차단, 일부 관측 가능)*
...
> ★ **rev.2 — D6 은 부분적으로 지금도 관측할 수 있습니다.**
> D6-1(Stage Manager 리사이즈 여부) · D6-3(Split View 진입 여부) · D6-6(회전) 은
> **"앱이 어떤 창 형태를 허용하는가" 를 보는 것**이라 **App Store 판으로도 관측됩니다.**
> 이미 설치돼 있는 "새기다" 를 Stage Manager 에서 열어 보기만 하면 됩니다 — **설치도 서명도 필요 없습니다.**
> 이 관측만으로도 **§11 기준 3 의 적용 가능 여부**라는 D6 의 핵심 산출물은 얻을 수 있습니다.
>
> 다만 D6-2(리사이즈 후 필사 위치 유지) · D6-4(왼손잡이 전환 좌표 정합) 는
> **실사용 데이터가 든 App Store 판에서 필기를 시도하게 되므로 권하지 않습니다.** 재결제 후로 미루십시오.
> 🚫 이 문서는 D6 을 공식적으로는 차단으로 둡니다. 위 관측은 **비공식 조기 확인**으로만 쓰고,
> 결과를 §8-3 에 적을 때 **"App Store 판으로 관측" 이라고 반드시 명시**하십시오.

**D7 — 차단됨(산출물 부재) / D7-pre — 지금 가능** *(rev.1 판정 · rev.2: D7-pre 도 🚫 차단)*
...
- ~~추출 명령: `devicectl device copy from --domain-type appDataContainer`~~ → **rev.2 에서 폐기**.
  이 명령은 **개발 서명으로 설치된 앱**의 컨테이너만 봅니다. 추출 대상은 App Store 판이므로 애초에 대상 밖입니다 (§0-A ②).
- **새 경로:** Finder 로컬 **비암호화** 백업 → `Manifest.db`(평문 SQLite) 조회 → 해시 파일을 원래 트리로 복원.
- 필요한 도구가 전부 이 머신에 있음 ✅ 확인: `/usr/bin/sqlite3` **3.54.0**, `/usr/bin/plutil`, `shasum`/`file`/`ditto`.
- 앱 데이터가 백업에 포함됨 ✅ 확인: 저장소 전체에 `NSURLIsExcludedFromBackupKey` / `isExcludedFromBackup` **0건**.
- 추출 대상 파일명 ✅ 확인: **`Carve.sqlite`** — `SwiftDataContextProvider.swift:26` 의 `id.contains("dev")` 분기에서
  App Store(Release) 판은 `dev` 가 없으므로 `Carve.sqlite` 입니다. (`Carve.dev.sqlite` 는 Debug 전용이며 백업에 없을 가능성이 높습니다.)
- 저장 위치 ✅ 확인: `SwiftDataContextProvider+Dependency.swift:29` 가 `URL.applicationSupportDirectory` 를 씁니다
  → 백업의 `relativePath` 는 **`Library/Application Support/Carve.sqlite`** 형태입니다.
- ⚠️ **기기에 쓰는 단계가 하나도 없습니다.** rev.1 의 "되돌릴 수 없는 설치 단계" 가 사라졌습니다.
- ⚠️ 남은 제약은 **TCC(전체 디스크 접근 권한)** 하나입니다 (§1-2-a).

> ★ **§18-4 의 경로 (a)·(b) 둘 다 정정이 필요합니다**
> §18-4 는 확보 경로를 `(a) 실기기에서 dev CloudKit 컨테이너로 내려받기` / `(b) 파일 복사` 로 적었습니다.
> - **(a) 는 여전히 무효**입니다. Debug 빌드가 붙는 곳은 **dev 컨테이너**이고 실사용 데이터는 **prod 컨테이너**에 있습니다
>   (`Project.swift:36` vs `:40`). dev 컨테이너를 채워 둔 적이 없다면 아무것도 내려오지 않습니다.
> - **(b) 도 rev.1 이 해석한 방식(`devicectl` 로 기기에서 직접 복사)으로는 무효**입니다 (§0-A ②).
> - **성립하는 것은 (b) 를 "백업 사본에서 파일 복사" 로 다시 읽는 것**이고, rev.2 의 D8 이 그것입니다.

> ★ **마이그레이션이 블롭을 보존한다는 점은 그대로 유효합니다**
> 경로가 바뀌어도 이 근거는 변하지 않습니다 — 아래 별도 블록을 그대로 보십시오.
...
> 이것이 설계 §10-2 가 원하는 것입니다.

---

## 4. 실행 순서 (rev.2)

### 4-1. 지금 실행하는 것 — **D8 하나뿐입니다**

| 순서 | 항목 | 이걸 하면 풀리는 것 | 지금 아니면 못 하는가 |
|---:|---|---|---|
| **1** | **§2-4 비암호화 백업** | D8 의 **추출 원본.** 이것 없이는 D8 자체가 성립하지 않습니다 | — |
| **2** | **§1-2-a FDA 확인** (또는 §1-2-a 의 GUI 경로) | 백업 디렉터리를 셸에서 읽을 수 있게 함 | — |
| **3** | **D8** — legacy `lineData` 추출 (§6-1) | **S5 fixture 확보 → Phase 0B 의 회귀 판정 기준이 열림.** 지금 Phase 0B 의 "실제 legacy 샘플 fixture" 항목이 입력 없이 막혀 있습니다 | ⚠️ **예.** 아래 참조 |

> ⚠️ **D8 이 여전히 "지금 아니면 못 한다" 인 이유 — rev.1 과 내용이 달라졌습니다.**
> rev.1 의 이유는 "곧 덮어 설치할 것이므로 그 전에 꺼내야 한다" 였습니다. **그 압박은 사라졌습니다.**
> rev.2 에 남은 이유는 하나입니다 — 기기를 초기화하거나, 앱을 삭제하거나,
> 앱이 언젠가 스키마를 다시 마이그레이션하며 블롭을 재인코딩하면
> **1.2.0 시절 인코딩의 원본이 사라집니다.** 그러면 설계 §10-2 가 원하는 "진짜 legacy 샘플" 을 어디서도 만들 수 없습니다.
> 시급성은 낮아졌지만 **비가역성은 그대로**입니다.

### 4-2. 🚫 재결제 이후로 이월 — 순서 자체는 rev.1 그대로 유효

**재개 조건:** 유료 Apple Developer Program 재결제 + Apple Development 인증서 재발급
→ §5 P0-3 에서 서명이 실제로 통과하는지 확인 → P0-5 빌드 → P0-6 설치

| 순서 | 항목 | 이걸 하면 풀리는 것 | 이월 사유 |
|---:|---|---|---|
| **0** | **P0 준비** (백업 → 서명 확인 → 빌드 → 설치) | 이후 전부의 전제 | 🚫 서명 불가 |
| **1** | **D5 baseline** (N-Canvas, 실기기) | **Phase 2 의 `LazyVStack → VStack` 판정 기준.** §13 이 S0-4 를 "특히 중요" 라고 한 것과 **같은 논리** — 바꾼 뒤에는 기존 경로를 다시 잴 수 없습니다 | 🚫 Instruments 는 개발 서명된 앱에만 부착됩니다 |
| **2** | **D7-pre** — dev CloudKit 미러링 동작 확인 | Phase 1 착수 판단. §18-5 1번이 남긴 공백을 메움 | 🚫 CloudKit entitlement 서명이 바로 그 불가능한 부분 |
| **3** | **D6** — Stage Manager / 리사이즈 | **§11 기준 3 이 적용 가능한 기준인지** 결정 → S4 통과 기준표가 확정됨 | 🚫 단, **일부는 지금도 관측 가능** (§3-2 D6 의 rev.2 주석) |
| **4** | **D3** — 실제 Pencil 지우개 | 설계 §7-3 승계 전략 / 설계 §7-4 `.bitmap` 확정을 시뮬레이터 결론에서 실기기 결론으로 승격 | 🚫 설치 불가 |
| **5** | **D4** — 두 제스처 | 현재 배선의 실동작 확인. undo 결함이 실기기에서도 재현되면 §13 의 `SharedUndoManager` 제거 근거가 실증됨 | 🚫 설치 불가 |
| **6** | **D1 · D2** — Pencil 입력 / 스크롤 UX baseline | S4 spike 설계 입력(무엇을 A/B 로 비교할지) | 🚫 설치 불가 |
| **7** | **D7** | — | **차단 (산출물 부재).** Phase 1 이후. **결제와 무관합니다** |

> ⚠️ **D5 의 비가역성은 이월한다고 사라지지 않습니다.**
> D5 는 "Phase 2 로 구조를 바꾸기 전에만" 잴 수 있는 baseline 입니다.
> **재결제가 Phase 2 착수보다 늦어지면 D5 baseline 은 영구히 못 잽니다.**
> → **Phase 2 착수 전에 재결제를 끝낼 것인가, 아니면 D5 없이 Phase 2 를 진행할 것인가**를 명시적으로 결정하십시오.
> 이건 이 런북이 아니라 **설계·일정 쪽의 판단**이므로 §8-6 에 올려 두십시오.

### 4-3. 이 재분류가 후속 Phase 에 주는 영향

| Phase | 영향 |
|---|---|
| **Phase 0B** | D8 만 있으면 됩니다. **영향 없음** — 오히려 D8 경로가 안전해져 착수가 쉬워졌습니다 |
| **Phase 1 (V4 스키마)** | D7-pre 없이 착수하게 됩니다. CloudKit 제약은 **코드 리뷰·문서로만** 판단해야 하며, 이는 §18-5 1번이 이미 리스크로 적어 둔 상태 그대로입니다 |
| **Phase 2 (VStack 전환)** | ⚠️ **D5 baseline 없이 진행할 위험.** 위 경고 참조 |
| **S4 spike** | D1·D2 실기기 baseline 없이 설계해야 합니다. 비교 재료가 시뮬레이터 측정만 남으므로 §7-2 의 제약이 그대로 적용됩니다 |

---

## 5. P0 — 공통 준비

> ★ **rev.2 — 이 절의 절반이 지금은 실행되지 않습니다.**
>
> | 단계 | rev.2 상태 | 비고 |
> |---|---|---|
> | P0-0 백업 | ✅ **수행** | 역할이 "안전망" → **"D8 의 추출 원본"** 으로 바뀌었습니다 (§2-4). **비암호화**로 뜨십시오 |
> | P0-1 툴체인 고정 | ✅ **수행** | D8 에도 필요합니다 (저장소 상태 확인) |
> | P0-2 프로젝트 생성 | — **불필요** | D8 은 빌드하지 않습니다 |
> | P0-3 Signing 확인 | 🚫 **차단** | 인증서 폐기 + 유료 멤버십 만료 |
> | P0-4 기기 인식 | — **불필요** | D8 은 기기를 연결하지 않습니다 (백업만 있으면 됩니다) |
> | P0-5 빌드 | 🚫 **차단** | P0-3 종속 |
> | P0-6 설치 | 🚫 **차단** | P0-5 종속. rev.1 의 "되돌릴 수 없는 단계" 가 여기였습니다 |
> | P0-7 실행 | 🚫 **차단** | P0-6 종속 |
>
> **D8 만 하려면 P0-0 · P0-1 두 개면 충분하고, 곧바로 §6-1 로 가면 됩니다.**
> 🚫 단계들은 **삭제하지 않았습니다** — 재결제 후 그대로 실행할 절차입니다.

### P0-0. ⚠️ 백업 (§2-4) — ✅ 지금 수행

**먼저 하십시오.** 완료 전에는 아래 어떤 단계도 실행하지 마십시오.

> ★ **rev.2 에서 이 단계의 성격이 바뀌었습니다.**
> rev.1: "설치가 데이터를 지울 경우를 대비한 안전망" → **하지 않아도 D8 은 돌아감**
> rev.2: **"D8 이 읽을 원본 그 자체"** → **하지 않으면 D8 이 아예 성립하지 않음**
> 그리고 **암호화가 아니라 비암호화**로 떠야 합니다. 근거는 §2-4.
...
> 🚫 **차단 (설치 불가).**
> **사유:** Apple Development 인증서가 폐기됐고 유료 멤버십이 만료/미결제 상태입니다.
> `Carve.entitlements` 가 요구하는 CloudKit entitlement 는 유료 멤버십 전용이라
> 무료 Personal Team 으로는 프로비저닝 프로파일이 만들어지지 않습니다 (§0-A ①, §3-1-a).
> **재개 조건:** 유료 Apple Developer Program 재결제 + Apple Development 인증서 재발급.
> **재개 시 첫 검증:** 아래 절차를 그대로 수행하되, 4번 항목에서 **Team 과 프로비저닝 프로파일에 오류 표시가 없는지**
> 를 반드시 확인하십시오. 여기서 통과하면 §3-1 의 차단 7건이 한꺼번에 풀립니다.
...
> 🚫 **차단 (설치 불가).** **사유:** P0-3 종속 — 서명이 안 되면 `-allowProvisioningUpdates` 도 실패합니다.
> **재개 조건:** P0-3 통과.
...
> 🚫 **차단 (설치 불가).** **사유:** P0-5 종속.
> **재개 조건:** P0-5 통과 **+ 그 직전에 §2-4 백업을 다시 뜰 것.**
> ⚠️ **재개 시 §2-1 · §2-3 의 경고가 그대로 되살아납니다.** 이 단계가 rev.1 에서 유일하게 비가역이었던 곳입니다.
> **rev.2 의 D8 은 이 단계를 통과하지 않습니다** — 백업에서 읽으므로 필요가 없습니다 (§6-1).
...
> 즉 **App Store 로 설치된 앱의 컨테이너는 내려받을 수 없습니다.**
>
> ★ **rev.2 — 이 한 문장이 rev.1 의 D8 을 무너뜨렸습니다.**
> rev.1 은 여기까지 정확히 인용해 놓고 결론을 *"그러니 D8 은 설치를 먼저 해야 한다"* 로 냈습니다.
> 하지만 D8 이 원하는 것은 **App Store 판이 쌓아 온 실사용 데이터**입니다.
> 덮어 설치하면 그 설치 상태가 사라지고, 컨테이너가 보존되는지도 ❓ 미확인이었습니다 (§2-3 1번).
> **가정이 참이어도 틀린 경로였습니다.** rev.2 의 D8 은 백업에서 읽습니다 (§6-1).

### P0-7. 🚫 실행

> 🚫 **차단 (설치 불가).** **사유:** P0-6 종속. **재개 조건:** P0-6 통과.
...
> ★ **rev.2 — 이 절에서 지금 실행할 수 있는 것은 §6-1 (D8) 하나입니다.**
> §6-2 ~ §6-8 은 전부 🚫 **차단 (설치 불가)** 이며 **재결제 후 그대로 실행**할 절차입니다 (§3-1, §4-2).
> 내용은 손대지 않았습니다 — 차단 사유가 절차의 옳고 그름과 무관하기 때문입니다.

---

### 6-1. D8 — 진짜 legacy `lineData` 1회 추출 ★ 지금 실행 가능한 유일한 항목

**푸는 것:** S5 fixture → Phase 0B 회귀 판정 기준

> ★ **rev.2 에서 경로를 전면 교체했습니다.** 무엇이 왜 바뀌었는지 먼저 읽으십시오.

#### 6-1-0. 왜 `devicectl` 경로를 버렸나

| # | rev.1 의 전제 | rev.2 의 사실 |
|---|---|---|
| 1 | "개발 빌드를 설치한 뒤 `devicectl device copy from` 으로 컨테이너를 받는다" | 🚫 **설치 자체가 불가능합니다.** `Carve.entitlements` 의 CloudKit entitlement 는 유료 멤버십 전용이고, 인증서가 폐기됐습니다 (§0-A ①) |
| 2 | "설치가 기존 컨테이너를 보존하면 `Carve.sqlite` 를 꺼낼 수 있다" | ❌ **그 가정이 참이어도 잘못된 경로였습니다.** `devicectl` 과 Xcode *Download Container…* 는 **개발 서명으로 설치된 앱**의 컨테이너만 봅니다 — rev.1 §5 P0-6 이 스스로 인용한 조건 (*Third-party / Installed via Xcode (profile validated) / Not enterprise signed*) 그대로입니다. 추출 대상은 **App Store 판**이므로 애초에 대상 밖입니다 |
| 3 | "위험은 설치 단계뿐이다" | ⚠️ 그 위험이 **필수 경로**였다는 것이 문제였습니다. 덮어 설치하는 순간 App Store 설치 상태가 파괴되고, 되돌리려면 재설치 + 동기화에 기대야 했습니다 |

**→ 성립하는 유일한 경로: Finder 로컬 백업에서 파일을 꺼낸다.**

| 이 경로의 성질 | |
|---|---|
| 기기에 **쓰지 않습니다** | 백업은 읽기 작업입니다. App Store 판은 손대지 않은 채 그대로 남습니다 |
| **설치하지 않습니다** | 서명·프로비저닝·인증서가 전혀 필요 없습니다 |
| **되돌릴 수 없는 단계가 없습니다** | rev.1 의 최대 위험이 통째로 사라졌습니다 |
| 필요한 도구가 **이미 다 있습니다** | `/usr/bin/sqlite3` 3.54.0 · `/usr/bin/plutil` — ✅ 이 머신에서 확인 |
...
### 6-2. 🚫 D5 — 시편 119편 실측 (N-Canvas baseline)

> 🚫 **차단 (설치 불가).** Instruments 는 **개발 서명된 앱**에만 부착됩니다. **재개 조건:** §5 P0-6 통과.
> ⚠️ **Phase 2 착수 전에만 잴 수 있는 baseline 입니다** — 일정 리스크는 §4-2 를 보십시오.
...
> 🚫 **차단 (설치 불가).** dev 컨테이너에 붙으려면 **CloudKit entitlement 가 채워진 개발 서명**이 필요한데,
> 바로 그것이 불가능한 부분입니다 (§0-A ①). **재개 조건:** §5 P0-6 통과.
...
> 🚫 **차단 (설치 불가) — 단, D6-1 · D6-3 · D6-6 은 App Store 판으로도 관측 가능**합니다.
> 예외의 범위와 주의점은 §3-2 의 D6 rev.2 주석을 보십시오. **재개 조건:** §5 P0-6 통과.
...
> ⚠️ **rev.2 — 6번의 추출 방법이 D8 과 더 이상 같지 않습니다.**
> rev.1 은 "D8 2단계와 동일한 명령" 이라고 적었지만, **D8 은 백업 경로로 바뀌었습니다** (§6-1).
> 반면 D3 의 대상은 **개발 빌드가 만든 `Carve.dev.sqlite`** 이므로,
> **재개 시점에는 `devicectl device copy from` 또는 Xcode *Download Container…* 가 오히려 적절합니다.**
> 개발 서명으로 설치한 앱이라 컨테이너 접근 조건을 만족하기 때문입니다.
> D3 을 위해 백업을 뜰 필요는 없습니다 (dev DB 는 실사용 데이터가 아닙니다).
> 🔌 명령 형태는 rev.1 의 §5 P0-6 / 구 §6-1 에 있던 것을 그대로 쓰되, 실행 결과는 미검증입니다.
...
> 🚫 **rev.2 — D5 는 지금 차단돼 있습니다** (§3-1). **이 절의 방법론은 손대지 않았습니다.**
> 차단 사유(개발 빌드 설치 불가)는 측정 방법의 옳고 그름과 무관하고,
> 재결제 후 그대로 실행할 내용이기 때문입니다. §7-2 의 "시뮬레이터 수치와 직접 비교하지 말 것" 은
> **D5 를 못 재는 지금 오히려 더 중요합니다** — 비교 재료가 시뮬레이터 측정만 남기 때문입니다 (§4-3).
...
| **⚠️ 백업 완료 여부 / 시각 (기기 A)** | |
| **⚠️ 백업 완료 여부 / 시각 (기기 B)** | |
| ★ **백업 암호화 여부** (`IsEncrypted`) — **`false` 여야 합니다** | |
| ★ 백업 디렉터리 (UDID) / `Last Backup Date` | |
| ★ **터미널 FDA 부여 여부** (§1-2-a) | |
| ★ `sqlite3` 버전 (`/usr/bin/sqlite3 --version`) | |
| 백업 시점의 App Store 판 버전 | |
| 🚫 설치한 Debug 빌드 configuration / 번들 ID | *(rev.2: 설치하지 않음)* |
| 🚫 설치 후 기존 데이터 컨테이너 보존 여부 | *(rev.2: 설치하지 않으므로 해당 없음)* |

> ★ = rev.2 에서 추가된 항목 (백업 추출 경로).

### 8-2. D1~D8 판정 요약 (실행 후 갱신)

| ID | rev.2 사전 판정 | 실제 실행 여부 | 결과 (통과 / 실패 / 보류 / 미실행) | 비고 |
|---|---|---|---|---|
| D1 | 🚫 차단 (설치 불가) | | | 재결제 후 |
| D2 | 🚫 차단 (설치 불가) | | | 재결제 후 |
| D3 | 🚫 차단 (설치 불가) | | | 재결제 후 |
| D4 | 🚫 차단 (설치 불가) | | | 재결제 후 |
| D5 | 🚫 차단 (설치 불가) | | | ⚠️ Phase 2 착수 전까지만 유효 |
| D6 | 🚫 차단 (설치 불가) | | | D6-1/3/6 은 App Store 판으로 부분 관측 가능 — 관측 시 **그 사실을 명시** |
| D7 | 차단됨 (산출물 부재) | | | Phase 1 이후. 결제와 무관 |
| D7-pre | 🚫 차단 (설치 불가) | | | 재결제 후 |
| **D8** | ✅ **지금 가능** (백업 경로) | | | |

### 8-3. 항목별 상세 기록

**D8 — legacy `lineData` 추출 (rev.2: 백업 추출 경로)**

| # | 판정 기준 | 관측 | 판정 |
|---|---|---|---|
| D8-0 | 백업이 비암호화 (`IsEncrypted = false`, `Manifest.db` = SQLite) | | |
| D8-1 | `AppDomain-kr.co.carve.leetaek` 도메인에 `Library/Application Support/Carve.sqlite` 존재 | | |
| D8-2 | 추출 누락 0건 + `Carve.sqlite` 가 `sqlite3` 로 열림 | | |
| D8-3 | 드로잉 테이블 행 ≥ 1, 블롭이 있는 행 존재 | | |
| D8-4 | `ZDRAWINGVERSION` 이 `1` 또는 `NULL` 인 행 존재 | | |
| D8-5 | `ZCREATIONDATE` 최솟값이 1.2.0 배포 이전 | | |
| D8-6 | `.externalStorage` 블롭 누락 없음 (또는 "전부 인라인" 확인) | | |

**S5 fixture 성립 여부:** D8-0·1·2·3·6 통과 + (D8-4 또는 D8-5) → ______
...
| 덤프 저장 위치 (저장소 밖) | |
| 복사한 파일 수 / 디렉터리 수 / **누락 수** | |
| `.tables` 실제 출력 (테이블명) | |
| 드로잉 테이블 행 수 | |
| 블롭 길이 분포 (최소 / 중앙 / 최대) | |
| `_EXTERNAL_DATA` 파일 수 / 총 크기 | |
| `ZDRAWINGVERSION` 분포 | |
| `ZCREATIONDATE` 범위 (UTC) | |
| fixture 후보로 고른 블롭 (개수 / 길이 / 인라인·external 여부) | |
| 개인정보 제거 조치 | |
| 덤프 사후 처리 (삭제 / 이동 위치) | |
...
| # | 발견 | 영향 | 반영할 곳 |
|---|---|---|---|
| **R1** | *(rev.2 선반영)* `devicectl` / Download Container 는 **App Store 설치 앱의 컨테이너에 접근할 수 없다** | rev.1 D8 의 전제가 틀렸음. 백업 경로로 교체 | 이 문서 §6-1 — **반영 완료** |
| **R2** | *(rev.2 선반영)* 유료 멤버십 부재로 **D1~D6 · D7-pre 가 전부 차단** | 설계 §18-5 의 "실기기에서 확인" 이월 항목들이 더 미뤄짐 | 설계 §18-5 / §13 리스크표 |
| **R3** | *(rev.2 선반영)* ⚠️ **D5 baseline 을 Phase 2 착수 전에 못 잴 가능성** | Phase 2 의 `LazyVStack → VStack` 판정 근거가 시뮬레이터 측정만 남음 | **일정 판단 필요** — §4-2 참조 |
| 4 | | | |
| 5 | | | |
...
### 9-2. 🚫 D1~D7-pre 를 수행한 경우 (재결제 이후)

> 🚫 지금은 해당 없음. **설치를 재개했을 때** 아래를 그대로 수행하십시오.
...
### 10-1. D8 (백업 추출 경로) — rev.2 에서 새로 생긴 항목 ★

**이 문서는 백업을 실제로 뜨지 않았고 기기를 연결하지도 않았습니다.**
아래는 전부 **실제 백업이 있어야만** 확인됩니다.
...
### 10-2. 차단 판정 자체에 대한 미확인 ★

| # | 항목 | 상태 |
|---|---|---|
| C1 | **CloudKit entitlement 가 무료 Personal Team 에서 정말 거부되는가** | ❓ **실증하지 못했습니다.** 인증서가 이미 폐기돼 서명 시도 자체가 불가능합니다. §3-1 의 차단 7건이 **이 전제 하나에 걸려 있습니다.** 재결제 전에 실제로 서명을 한 번 시도해 보는 것이 가장 싼 검증입니다 (§3-1-a) |
| C2 | D6-1/3/6 을 App Store 판으로 관측하는 것이 실제로 되는가 | 🔌 미검증. 앱을 열어 보기만 하면 되므로 위험은 없습니다 (§3-2 D6) |

### 10-3. rev.1 부터 남아 있는 항목 (설치 재개 시 다시 유효)

| # | 항목 | 왜 |
|---|---|---|
| 1 | **개발 서명 빌드 설치가 기존 데이터 컨테이너를 보존하는가** | ❓ 실기기 없이는 실증 불가. **rev.2 에서 D8 의 전제 조건은 아니게 됐지만**, D1~D6 재개 시 데이터 안전의 핵심 변수로 남습니다 (§2-3) |
| 2 | ~~`devicectl device copy from --source` 의 `appDataContainer` 상대 경로 해석~~ | **rev.2 에서 D8 과 무관해졌습니다.** D3 재개 시에만 필요합니다 (§6-5) |
...
| 6 | Stage Manager 에서 세로 고정 + scene manifest 부재 앱이 어떻게 처리되는가 | ❓ D6 이 답할 질문. 일부는 App Store 판으로도 관측 가능 |
| 7 | prod CloudKit 컨테이너에 실사용 데이터가 **전부** 올라가 있는가 | ❓ 확인 수단 없음. 백업을 필수로 두는 이유 |
| 8 | Apple 이 제시하는 hitch time ratio 절대 임계값 | ❓ 이 문서에서 단정하지 않습니다. A/B 상대 비교로 판정 |
| 9 | ~~백업에서 앱 컨테이너만 추출하는 CLI 경로~~ | ✅ **rev.2 에서 해소.** libimobiledevice 없이 **`/usr/bin/sqlite3` 만으로** 성립합니다 (§6-1). rev.1 이 "경로 없음" 이라고 적은 것은 **틀렸습니다** |
