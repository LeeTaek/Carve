...
> △ **D9-6 은 부분 통과입니다** — **R23**(flag off 로 장을 **열기만 해도** v3 절이 v2 로 강등되고 metadata 가 삭제됨, 설계 §10-3 위반)은 **수정하고 실기기에서 재확인했습니다**(회귀 2건, 기준선 302 → **304**). 남은 것은 **R24**(flag off 시점에 화면 밖이던 절의 잉크 미표시) 하나이며 **기본 활성화 차단 사유로 남습니다.** 이전 ✅ 는 화면만 보고 DB 를 안 본 판정이었습니다.
...
> · 회귀 기준선 **304** (설계 §19-4-2). **아래 기록에 남은 265 · 296 등은 그 시점의 이력이며 현재 기준선이 아닙니다.**
...
> △ **D9-6 부분** (R23 수정 완료 · **R24 미해결**) · **D9-5 · D9-7 나머지 · D9-CK③ ❓ 미수행** (§8-7).
>
...
| 회귀 기준선 | **304** (설계 §19-4-2) | ⚠️ 이 표가 작성될 당시(rev.5)는 265 였습니다. **줄어들면 회귀입니다** |
...
> | **D9-2** 지우개 | ✅ **통과** |
> | **D9-6** flag 토글과 롤백 | △ **부분** — R23(강등)은 ✅ **수정·실기기 재확인 완료**. **R24**(flag off 시점에 화면 밖이던 절의 잉크 미표시)가 **남아 있다.** 이전 ✅ 는 화면만 보고 DB 를 안 본 판정이었다 |
...
| 백업 (§2-4) | ❓ 미기록 |
| 실기기 빌드·설치·실행 | ✅ 성공 |
| 회귀 기준선 | **304** (설계 §19-4-2). ⚠️ 아래 기록에 남은 265 · 296 은 그 시점의 이력입니다 |
...
| **R24** | ⚠️ **미해결 — R23 과 별개 결함으로 확정.** **flag off 시점에 화면 밖이었던 절의 잉크가 표시되지 않는다.** R23 수정 후에도 그대로 재현된다. 재현 조건(2026-09-08, 시편 122편 9절): 토글 off 직후 **가로·세로 모두** 안 보임 · 그 절까지 **스크롤해도** 안 보임 · **회전하면** 나타남 · **다른 장 갔다 오면** 나타남. 살아나는 두 조작은 **행 집합을 다시 짓는다**는 공통점이 있고 스크롤은 그렇지 않다 | **원인 미규명.** 9절은 `v2` 행이라 `displayTransform`(v3 전용) 가설로는 설명되지 않는다. 데이터는 살아 있어 ⛔ 중단 기준("flag off 에서 데이터가 안 보임")에는 걸리지 않으나 **사용자에게는 필기가 사라진 것으로 보인다.** 롤백 경로라 기본 활성화 전에 닫아야 한다 | 설계 §10-3 · 이 문서 §6-9 D9-6 |
