...
| `Δ max` | **0.00** (초록, tol 1.00pt) | 0 이 아니면 **레이아웃보다 측정 경로를 먼저 의심** (AGENTS.md — 중첩 호스팅의 `.named` → global 대체) |
...
### 8-7. D9 실측 기록 (rev.5 — **실행 중, 스모크에서 결함 1건**)
...
> ⛔ **원인 확정 전까지 D9-1(필기) 을 시작하지 마십시오.** 하단 절이 잘못 귀속되고,
> 그것을 저장 경로 결함으로 오진하게 됩니다.
...
| # | 발견 | 영향 | 반영할 곳 |
|---|---|---|---|
| **R13** | ★ **실기기에서 `ChapterLayout` 예측이 절당 정확히 0.5pt 씩 실측과 어긋나 176절에서 87.5pt 누적.** 방향·폰트·경로(N-Canvas/단일 Canvas) 전부와 무관하고, **장 길이에 선형**(창세기 1장 31절 → 15.00). 0.5pt = @2x 의 1픽셀 | **Phase 3 이 아니라 Phase 2 의 레이아웃 예측 결함.** 기본 활성화 차단. 허용치 상향은 G3 포기와 같으므로 선택지가 아님 | 설계 §6 · §15 리스크표 · §16 D9. **수정 방향 결정 후 기재** |
