...
> ⚠️ **출시 전 판단이 필요한 것:** **R25**(롱프레스 메뉴 교체 — 오탭이 전 획 선택으로) · **R22**(세로에서 redo 접근 불가) · **R28**(구버전 기기가 `drawingVersion == 3` 을 오해석할 수 있음 — 현재 출시 빌드가 V4 인지 미확인) · **R27**(기존 행의 `rowUUID` 가 서버에 없어 중복 가능) · **R20**(저메모리 미검증).
...
| ⚠️ **R28** | **혼재 버전에서 구버전이 `drawingVersion == 3` 을 오해석할 수 있습니다.** 표식은 이미 Production 에 있던 필드라 동기화되는데 `layoutMetadataData` 는 구버전 모델에 없어 무시됩니다 — 해석에 필요한 정보만 빠집니다. 위험 여부는 **현재 App Store 빌드가 V4 인지**에 달렸고 미확인입니다 (런북 §8-7) |
