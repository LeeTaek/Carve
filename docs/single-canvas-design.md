...
> ⚠️ **기본 활성화 전 남은 것은 결함이 아니라 미수행 항목입니다** — **D9-CK③**(dev → Production 스키마 승격, 되돌릴 수 없어 사람이 직접) · **D9-5**(히스토리 메뉴) · **D9-7 나머지**(fling·탭/롱프레스) · **R20**(저메모리 기기에서 전이 peak 재측정).
...
> 회귀 기준선 **310** (§19-4-2). 대상: `Feature/CarveFeature`, `Feature/SettingsFeature`, `Domain`.
>
...
| ID | 내용 |
|---|---|
| ⛔ **R26** ★ | **긴 장을 떠나도 메모리가 회수되지 않습니다 (2026-09-09).** 시편 120편에 cold launch 로 들어가면 **179.7 MB**, 시편 119편을 거쳐 오면 **945 MB** — 765 MB 잔류. 짧은 장끼리(120→121)는 184.8 MB 로 누적이 없습니다. **런북 §6-9 의 중단 기준 항목**이며 D5(N-Canvas)는 전환 후 109.7 MB 로 평평했습니다. 저메모리 위험의 실제 주범은 회전 전이보다 이쪽으로 보입니다 — 전이는 간헐적이지만 잔류는 앱을 끌 때까지 남습니다. 유력한 설명은 장마다 재사용하는 `PKCanvasView` 의 내부 버퍼가 가장 큰 콘텐츠에 맞춰 잡힌 뒤 줄지 않는 것이지만 **미검증 가설**입니다 |
...
| **30** | **310** | **현재 실측 기준선** — 단일 Canvas 기본 전환 회귀 2 (토글 미조작 사용자가 기본값을 본다 · 명시적 off 가 유지된다). Swift Testing 308 + XCTest 2 |
