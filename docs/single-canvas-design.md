...
>
> ✅ **종결:** R13(절당 0.5pt 누적) · R16(장 전환 컬럼 신장) · R19(v3+metadata없음 배치) — §20-13 · §20-14 · 런북 §8-7.
> ⚠️ **살아 있는 항목:** **D9 H**(회전 표시 — 정식 수정 적용 + 실기기 A/B 통과했으나 §6 잔여 검증이 남아 **종결 아님**) ·
> **E-4**(글꼴 변경 시 잉크 재배치 — **미확정**, D9 H 잔여 검증에서 재판정) · **R17**(Δ 가 R16 계열 미검출) · **R18**(N-Canvas 신규 행이 v1). 전부 §16 "남은 것".
> D9-1 · D9-2 · D9-6 · D9-CK② 는 통과했고 **D9-3 나머지 · D9-4 · D9-5 · D9-7 · D9-8 · D9-CK③ 은 미수행**입니다 (런북 §8-7).
...
**종결된 결함 3건** — 재발 판별에 쓸 실측 기준만 남깁니다.
...
| ID | 내용 |
|---|---|
| ⚠️ **D9 H (미종결)** | 회전 후 이전 필기 표시. **정식 수정 적용 + 실기기 A/B 통과** (§20-16, `ef053111`) — 인자 없이 3왕복 정상, `-CanvasReuseStrokesOnApply` 로 결함 재현. **§6 잔여 검증이 남아 종결하지 않습니다** — 편집·저장 왕복 · 회전 직전 flush 의 실기기 확인 · 긴 장 성능 · 스크롤 중 회전 · 글꼴 변경. [분석 §6](./single-canvas-rotation-display-investigation.md) |
| ⚠️ **E-4 (미확정)** | 폰트·행간 변경 시 잉크가 재배치되지 않았다. rev.22 의 "결함 아님(v1 행이라서)" 판정은 **되돌렸습니다** — v1 은 band reflow 를 안 할 뿐이고, 폰트 변경과 회전은 같은 자극이라 D9 H 와 구분되지 않습니다. **D9 H 잔여 검증의 글꼴 변경에서 재판정** (§14 · 런북 §8-7) |
...
| ⚠️ **R18** | **N-Canvas 가 새로 만드는 행이 v2 가 아니라 v1(모델 기본값)로 저장됩니다.** 배치에는 영향이 없으나(코덱이 v1·v2 를 같게 처리) 진짜 legacy 와 오늘 그린 행이 `legacyVerses` 한 통에 섞여 **진단에서 구별되지 않습니다** — E-4 오독의 직접 원인. 저장 경로 별건 (런북 §8-7) |
| **D9 (진행 중)** | **절차: [런북 §6-9](./phase-0a-d-device-test.md) · 기록: 런북 §8-7.** 통과: 스모크 · **D9-1**(필기→저장→복원) · **D9-2**(지우개) · **D9-6**(flag 토글·롤백) · D9-CK②. 미수행: **D9-3 나머지**(reflow·undo 재실행) · **D9-4**(장 전환 flush) · **D9-5**(히스토리 메뉴) · **D9-7**(fling·탭/롱프레스·회전) · **D9-8**(성능, §18-3-a 절차) · **D9-CK③**(Production 승격). **기본 활성화의 선행 조건** |
