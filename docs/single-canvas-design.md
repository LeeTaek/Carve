...
>
> ✅ **종결:** R13(절당 0.5pt 누적) · R16(장 전환 컬럼 신장) · R19(v3+metadata없음 배치) · **D9 H**(회전 표시 — 실기기 잔여 검증까지 통과) · **E-4**(결함 아님, D9 H 와 동일 결함) · **R23**(flag off 로 열기만 해도 v3→v2 강등 — 수정·재확인 완료) — §20-13 · §20-14 · §20-16 · 런북 §8-7.
> ⛔ **기본 활성화 차단:** **R24**(flag off 시점에 화면 밖이던 절의 잉크가 표시되지 않음 — 스크롤로는 안 살아나고 회전·장 왕복 후에야 나타남). R23 수정 뒤에도 재현되는 **별개 결함**이며, D9 H 와 인과가 없는 N-Canvas 선재 결함입니다.
> ⚠️ **그 밖에 살아 있는 항목:** **R20**(재구성의 전이 메모리 peak 1.8 GiB) · **R21**(N-Canvas 대조 미측정) · **R17**(Δ 가 R16 계열 미검출) · **R18**(N-Canvas 신규 행이 v1) · **R22**(팔레트 폭). 전부 §16 "남은 것".
> D9-1 · D9-2 · D9-3-1·2·4 · D9-4-1·4 · D9-6-5 · D9-7-4 · D9-8 · D9-CK② 통과, **D9-6 은 부분**(R23 수정 완료 · R24 미해결), **D9-5 · D9-7 나머지 · D9-CK③ 미수행** (런북 §8-7).
...
> 회귀 기준선 **304** (§19-4-2). 대상: `Feature/CarveFeature`, `Feature/SettingsFeature`, `Domain`.
>
...
**종결된 결함 6건** — 재발 판별에 쓸 실측 기준만 남깁니다.
...
| ID | 내용 |
|---|---|
| ⛔ **R24** ★ **기본 활성화 차단** | **flag off 시점에 화면 밖이던 절의 잉크가 표시되지 않습니다.** R23 수정 후에도 그대로 재현됩니다 — **별개 결함**입니다. 토글 off 직후 가로·세로 모두 안 보이고, **그 절까지 스크롤해도** 안 보이며, **회전** 또는 **다른 장 왕복** 후에야 나타납니다(살아나는 두 조작만 행 집합을 다시 짓습니다). 원인 미규명 — 관측된 절이 `v2` 라 `displayTransform` 가설로는 설명되지 않습니다. 데이터는 살아 있으나 사용자에게는 필기가 사라진 것으로 보입니다 (런북 §8-7 R24) |
| ⚠️ **R20** | **표시용 획 재구성이 회전마다 큰 전이 메모리 peak 을 만듭니다.** 시편 119편에서 peak 기본 **1759.6 / 1817.8 MiB** vs 대조 **995.3 / 1173.6 MiB** (n=2씩, 교차 실행). 안정 상태는 같아 누수가 아니라 전이입니다. 대조가 적게 쓰는 이유는 **결함 자체**(캐시 재사용으로 렌더를 건너뜀)로 보이지만, **저메모리 iPad 는 미검증**입니다 (런북 §8-7 D9-8) |
| ⚠️ **R21** | 단일 Canvas 의 시편 119편 진입 peak(**944.8 MiB**)이 D5 의 N-Canvas baseline(**363.6 MB**)과 크게 다릅니다. 이번 세션은 수정 A/B 만 했고 **N-Canvas 대조를 뜨지 않았습니다.** 기본 활성화 전에 같은 절차로 재야 합니다 (런북 §8-7 D9-8 한계 5) |
...
| **26** | **304** | **현재 실측 기준선** — R23 회귀 2 (① 프로그램 대입은 v3 를 강등하지 않는다 ② 사용자 편집 콜백은 그대로 저장된다 — 억제 과잉 방지). 실측 내역: Swift Testing 302 + XCTest 2. 변이 테스트로 억제 제거 시 `drawingVersion → 2` · `metadata → nil` 실패 확인 |
