...
> ⚠️ **기본 활성화 전 남은 것은 결함이 아니라 미수행 항목입니다** — **D9-CK③**(dev → Production 스키마 승격, 되돌릴 수 없어 사람이 직접) · **D9-5**(히스토리 메뉴) · **D9-7 나머지**(fling·탭/롱프레스) · **R20**(저메모리 기기에서 전이 peak 재측정) · **R21**(N-Canvas 대조 미측정).
...
> 회귀 기준선 **308** (§19-4-2). 대상: `Feature/CarveFeature`, `Feature/SettingsFeature`, `Domain`.
>
...
**종결된 결함 7건** — 재발 판별에 쓸 실측 기준만 남깁니다.
...
| **28** | **308** | **현재 실측 기준선** — R24 회귀 4 (`ChapterLayoutReloadTesting`: 같은 장 유지 · 다른 장 폐기 · 절 목록 변경 시 폐기 · 실측 높이 유지). Swift Testing 306 + XCTest 2 |
