...
| ⚠️ **R29** | **`BiblePageDrawing` 이 코드에는 남아 있는데 CloudKit 레코드 타입은 없습니다 (2026-09-09).** 쓰기 호출부 0건, 실데이터 0행(D8), 유일한 읽기는 주석 처리된 `CombinedCanvasFeature` 에만 있습니다. 배포 전 정리 때 `CD_BiblePageDrawing` 을 지웠으므로 **이 경로를 되살리면 Production 에 없는 타입을 쓰게 되어 동기화가 조용히 실패합니다.** 되살리려면 CloudKit 배포가 먼저입니다 — `DrawingDatabase.upsertPageDrawing` 과 스키마에 경고 주석을 남겼습니다. **엔티티 제거는 V5 마이그레이션이 필요해 Phase 4** 입니다. 함께 죽어 있는 `CombinedCanvasFeature`·`CombinedCanvasView` 967줄도 Phase 4 정리 대상입니다 |
