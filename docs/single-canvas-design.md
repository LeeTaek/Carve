...
>
> ✅ **종결:** R13(절당 0.5pt 누적) · R16(장 전환 컬럼 신장) · R19(v3+metadata없음 배치) · **D9 H**(회전 표시 — 실기기 잔여 검증까지 통과) · **E-4**(결함 아님, D9 H 와 동일 결함) — §20-13 · §20-14 · §20-16 · 런북 §8-7.
> ⛔ **기본 활성화 차단:** **R23**(flag off 로 장을 **열기만 해도** v3 절이 v2 로 강등되고 metadata 삭제 — §10-3 위반) · **R24**(flag off 직후 화면 밖 절의 잉크 미표시). **둘 다 D9 H 와 인과 없는 N-Canvas 선재 결함입니다.**
...
> D9-1 · D9-2 · D9-3-1·2·4 · D9-4-1·4 · D9-7-4 · D9-8 · D9-CK② 통과, **D9-6 은 R23 으로 실패 정정**, **D9-5 · D9-7 나머지 · D9-CK③ 미수행** (런북 §8-7).
...
**종결된 결함 5건** — 재발 판별에 쓸 실측 기준만 남깁니다.
...
| ID | 내용 |
|---|---|
| ⛔ **R23** ★ **기본 활성화 차단** | **flag off 로 장을 열기만 해도 그 장의 v3 절이 전부 v2 로 강등되고 `layoutMetadataData` 가 삭제됩니다 — 사용자 편집이 없어도.** 2026-09-08 실기기 실측(시편 122편 9개 절). 획은 잃지 않습니다. 원인은 N-Canvas coordinator 에 **프로그램 대입 억제가 없어**(`CanvasView.canvasViewDrawingDidChange`) 로드 즉시 `.saveDrawing` 이 나가는 것 — 단일 Canvas 는 같은 자리를 `isApplyingDrawing` 으로 막습니다. **§10-3 위반**(설계는 "편집할 때만" 강등). **저장소가 forward-only 라 flag off 가 유일한 롤백 수단인데 그 경로가 데이터를 열화시킵니다.** ⚠️ D9 H(`ef053111`)와 인과 없는 **선재 결함**입니다 (런북 §8-7 R23) |
| ⛔ **R24** | **flag off 직후 화면 밖이던 절의 잉크가 표시되지 않습니다** — 스크롤로는 안 살아나고 회전(레이아웃 재계산) 후에야 나타납니다. 2회 관측·재현됨. 데이터는 살아 있습니다. R23 과 같은 뿌리로 보입니다 (런북 §8-7 R24) |
