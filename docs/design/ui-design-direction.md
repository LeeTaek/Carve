...
**보관 단위 제안.** 잘 쓴 필사를 모아두려는 목적에 맞춰 즐겨찾기에 추가한 당시의 본문·필기를 보존한다. 이후 원래 캔버스를 지우거나 다시 써도 보관된 필기는 바뀌지 않는다. 말씀만 즐겨찾기한 경우에는 필기 없는 카드로 보여준다. 이 스냅샷 정책은 구현 전에 확정할 사항이다. 시안은 번역본·절당 한 항목을 기본으로 하며 여러 필사 회차를 따로 보관하는 기능은 범위에 넣지 않았다.

**목록 조작 제안.** 카드 본문을 누르면 저장된 말씀과 필기를 크게 본다. `말씀으로 이동`은 해당 번역본·장·절의 필사 화면으로 이동하며, 저장된 필기를 현재 캔버스에 덮어쓰지 않는다. 카드의 채운 별은 44pt 해제 버튼이고 해제 직후 실행 취소를 제공한다. 즐겨찾기 해제는 원래 필사나 이전 필사 기록을 삭제하지 않는다. 상세 확대 화면은 이번 시안에 포함하지 않았다.
