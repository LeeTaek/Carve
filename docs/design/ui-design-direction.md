...
작성일: 2026-09-10. 현재 저장소 SVG 대조 갱신: 2026-09-11. [2.0.0 로드맵](../release-2.0.0-roadmap.md)의 DESIGN-0·1·2에서 사용할 디자인 검토 문서다. 대화에서 합의한 방향과 제작한 시안을 기록하며, 앱 구현 완료나 실기기 검증을 의미하지 않는다.

**현재 시안의 기준:** `assets/figma/*.svg` 39개가 편집 원본이고, 같은 이름의 `assets/figma-outlined/*.svg` 39개가 Figma 가져오기용이다. [프레임별 미리보기](assets/figma-preview.html)에서 확인한다. `assets/*.svg` 보드는 비교·설명용으로 일부 이전 배치가 남아 있다. 이 문서는 최신 프레임의 구현된 그림, 합의한 제품 동작, 아직 검증하지 않은 동작을 구분한다.
...
### 즐겨찾기 추가 시안 (2026-09-15)

#### 즐겨찾기와 위젯 연결 (2026-09-16 채택)

즐겨찾기를 보관함으로 사용하고, 그중 **한 말씀을 명시적으로 선택**해 위젯에 표시한다. 즐겨찾기 전체를 자동으로 순환하지 않는다. 말씀 관리는 즐겨찾기에서, 위젯 표시 설정과 추가 방법은 `SettingsFeature`의 `위젯` 화면에서 제공한다. 기존 안 G의 독립적인 위젯 보관 방식보다 이 결정을 우선한다.

| 시안 | 내용 |
|---|---|
| `n4-favorites-list.svg` 갱신 | 선택한 카드에 `위젯에 표시 중` 배지와 카드 더보기 버튼 표시 |
| `n6-widget-card-menu.svg` | 카드 더보기에서 `위젯에 표시`. 다른 말씀을 선택하면 현재 대상을 교체한다. 선택된 카드에서는 `위젯 표시 해제`로 바뀐다. |
| `n7-widget-settings.svg` | 설정 → 위젯. 현재 말씀, `표시할 말씀 바꾸기`, `위젯 표시 해제`, 홈 화면 추가 안내 |
| `n8-widget-picker.svg` | 즐겨찾기 재사용 선택 화면. 단일 선택 후 `적용`, 취소 시 기존 대상 유지. 즐겨찾기가 없으면 N5의 빈 상태 안내를 재사용한다. |
| `n9-widget-remove-confirm.svg` | 표시 중인 즐겨찾기 해제 시 위젯에서도 제외됨을 안내. 해제 후 다른 말씀을 자동 선택하지 않는다. |

롱탭의 `위젯에 표시`도 유지한다. 즐겨찾기에 없는 절은 현재 모습을 즐겨찾기에 보관하고 위젯 대상으로 선택하며 `즐겨찾기에 추가하고 위젯에 표시했어요`라고 알린다. 이미 보관된 절은 보관본을 사용하고 `위젯에 표시했어요`라고 알린다. 두 진입점은 동일한 선택 상태를 사용한다. 위젯 표시만 해제하면 즐겨찾기는 남는다. 대상이 없으면 위젯은 말씀 선택 안내를 보여준다.

위젯에는 보관한 당시의 본문·필기를 표시한다. 현재 캔버스 수정·지우기로 보관본이 바뀌지 않는다. 처음에는 공통 선택 한 개를 모든 새기다 말씀 위젯에 적용한다. 위젯별 개별 선택·여러 말씀 순환·변경 주기는 후속 범위다. 앱에서 선택하는 것과 홈 화면에 위젯을 설치하는 것은 별도이며, 실제 반영 시점은 시스템 갱신에 따른다.

새 화면은 [즐겨찾기·위젯 미리보기](assets/favorites-preview.html)에서 확인한다. 이번 변경은 SVG와 문서에만 적용하며 앱 코드 구현 상태와 구분한다.

마음에 드는 말씀과 잘 쓴 필사를 한곳에서 모아 보는 기능이다. 이름은 **즐겨찾기**로 통일한다. [시안 5종과 Figma 가져오기용 SVG](assets/favorites-preview.html)를 추가했다. 기존 시안은 보존하고 `n1`~`n5` 파일로 분리했다. 아래 내용은 디자인 제안이며 앱 구현 완료를 뜻하지 않는다.

| 화면 | 파일 | 표시와 동작 |
|---|---|---|
| 절 롱탭 | `n1-favorite-menu.svg` | 첫 항목에 빈 별과 `즐겨찾기에 추가`. 이미 추가한 절은 채운 별과 `즐겨찾기 해제`로 바뀐다. 기존 기록·이미지·위젯·지우기 항목을 유지한다. |
| 등록된 절 | `n2-favorite-marked.svg` | `SentencesWithDrawingView`의 절 번호 아래에 작은 채운 별을 표시한다. 본문·필기 폭과 행 높이는 바꾸지 않는다. 별은 상태 표시이고 해제는 롱탭 메뉴에서 한다. 추가 직후 `즐겨찾기에 추가했어요`를 잠깐 보여준다. |
| 탐색 사이드바 | `n3-favorite-navigation.svg` | 하단 차트 아이콘 바로 왼쪽에 44pt 별 버튼을 추가한다. 번역본·차트·설정과 기존 네이티브 광고 자리는 유지한다. 접근성 이름은 `즐겨찾기`다. |
| 목록 | `n4-favorites-list.svg` | 별 버튼을 누르면 진입한다. 최근 추가순으로 성경·장·절·번역본, 말씀 원문, 필기 미리보기와 추가 날짜를 보여준다. 필기 없는 말씀도 추가할 수 있다. |
| 빈 목록 | `n5-favorites-empty.svg` | `아직 즐겨찾기한 말씀이 없어요`와 절 롱탭 안내, `말씀 보러 가기` 버튼을 표시한다. |

각 파일은 `assets/figma/`에 편집용, `assets/figma-outlined/`에 나눔 펜 필기를 윤곽선으로 보존한 가져오기용이 있다. UI 문구는 해요체다.

**보관 단위 제안.** 잘 쓴 필사를 모아두려는 목적에 맞춰 즐겨찾기에 추가한 당시의 본문·필기를 보존한다. 이후 원래 캔버스를 지우거나 다시 써도 보관된 필기는 바뀌지 않는다. 말씀만 즐겨찾기한 경우에는 필기 없는 카드로 보여준다. 이 스냅샷 정책은 2026-09-16 위젯 연결 방향으로 채택했다. 시안은 번역본·절당 한 항목을 기본으로 하며 여러 필사 회차를 따로 보관하는 기능은 범위에 넣지 않았다.

**목록 조작 제안.** 카드 본문을 누르면 저장된 말씀과 필기를 크게 본다. `말씀으로 이동`은 해당 번역본·장·절의 필사 화면으로 이동하며, 저장된 필기를 현재 캔버스에 덮어쓰지 않는다. 카드의 채운 별은 44pt 해제 버튼이고 해제 직후 실행 취소를 제공한다. 위젯에 표시 중이면 N9 확인을 먼저 거친다. 즐겨찾기 해제는 원래 필사나 이전 필사 기록을 삭제하지 않는다. 상세 확대 화면은 이번 시안에 포함하지 않았다.

**검증 범위.** 신규 5종의 가져오기용 SVG를 PNG로 렌더해 배치를 확인했다. SVG XML 파싱과 문서 공백 검사를 통과했다. Figma 가져오기, 실제 터치, 저장·동기화 동작은 아직 검증하지 않았다.
...
| [i1-navigation-dim.svg](assets/figma/i1-navigation-dim.svg) | I1 대상 강조 · 성경 이동 | 1180 × 820 |
| [i1-scroll-dim.svg](assets/figma/i1-scroll-dim.svg) | I1 대상 강조 · 펜 필기와 손가락 스크롤 | 1180 × 820 |
| [i1-palette-dim.svg](assets/figma/i1-palette-dim.svg) | I1 대상 강조 · 팔레트 펼치기 | 1180 × 820 |
| [i1-longpress-dim.svg](assets/figma/i1-longpress-dim.svg) | I1 대상 강조 · 절 롱탭 | 1180 × 820 |
| [i1-image-dim.svg](assets/figma/i1-image-dim.svg) | 이전 I1 비교용 · 이미지 저장 | 1180 × 820 |
| [i1-widget-dim.svg](assets/figma/i1-widget-dim.svg) | 이전 I1 비교용 · 위젯에 표시 | 1180 × 820 |
...
기본 디자인에서 가져온 것 — 부제에 `개역개정 · 구약 / 시편` 을 함께 적고, 오른쪽 열 라벨을 `나의 필사 · 절을 길게 눌러 더 보기` 로 두어 롱탭을 그 자리에서 알린다. 절 번호는 첫 줄 위 위첨자로 올리고 절 사이를 30pt 띄운다.

현행 결정에서 얹은 것 — 헤더와 팔레트를 글자에서 아이콘으로, 팔레트를 하단 중앙 유리로, 필기 예시를 펜 글꼴로 바꿨다. 본문은 앱이 싣는 개역개정 표기대로 `쉴 만한 물 가로` 로 적었다 — 2026-09-21 에 개역한글 표기(`쉴 만한 물 가으로` · `푸른 초장` · `내가 부족함이`)에서 맞췄다.
...
| 항목 | 값 |
| --- | --- |
| 글꼴 | 나눔명조(기본) · 나눔바른고딕 · 나눔꽃향기 — `Domain.FontCase` |
| 글자 크기 | 15 ~ 40, 기본 20 |
| 줄 간격 | 5 ~ 70, 기본 30 |
| 자간 | 1 ~ 10, 기본 1 |
...
**변경 (2026-09-14, 사용자 결정).** `본문 설정` 버튼은 왼손 모드에서도 오른쪽에 고정한다. 서재 옆에 버튼이 늘면 헤더 광고(안 K)가 들어갈 자리가 사라지기 때문이다. 대가로 왼손 모드에서는 팝오버가 오른쪽 원문 열을 덮는다 — 위 문단의 이유와 반대이며 알려진 제약으로 둔다.
...
**최신 결정: 첫 사용 안내는 일반 팝업 하나로 표시한다.** 아래 네 항목을 한 팝업에 담고, `시작하기` 버튼으로 닫는다. 단계별 진행, 대상만 밝히는 강조, 화살표·손가락 애니메이션은 사용하지 않는다. 기존 I1 SVG와 [단계별 미리보기](assets/i1-spotlight-preview.html)는 이전 검토 자료이며 구현 기준이 아니다.
...
문구는 해요체를 사용한다. 펜은 필기, 손가락 드래그는 한 손가락을 포함한 스크롤이라는 최신 합의를 반영했다. 이전 SVG에 남은 두 손가락 전용 설명보다 이 문구가 우선한다. 이미지 저장·위젯 안내는 해당 기능이 제공되는 버전에 사용한다.
...
**구현 (2026-09-14).** 시안 K2 · K3 의 수치를 AdMob 네이티브 필수 요소에 맞춰 조정했다 — 광고 배지 「광고」(앱 언어로 옮겨 쓸 수 있고 15px 이상), 제목 한글 12~13자 · 영문 25자까지 잘리지 않음, CTA, 오른쪽 위 AdChoices 자리.

- **헤더 줄:** 폭 **184pt 고정**(세로 iPad mini 에서 가운데 제목과 서재 버튼 사이 폭. 가로 화면에서도 넓히지 않는다), 높이는 헤더 축소를 따라 **펼침 56pt ↔ 축소 44pt**. 서재 쪽 버튼과 16pt 떨어뜨렸다. 아이콘(32pt 이하) · 위에 배지 · 제목 두 줄(11pt) · CTA 구성이다. 좁으면 제목이 70pt 밑으로 줄기 전에 CTA 가 먼저 잘린다. 버튼 줄 중심선에 맞춰 위아래로 넘치므로 광고가 생기거나 높이가 바뀌어도 버튼 · 제목은 움직이지 않는다.
- **세로 화면에도 둔다(위 제약 둘째를 뒤집음, 사용자 결정).** 제목은 늘 가운데에 두고 광고는 서재 쪽 버튼 뒤에 둔다. 광고를 둘 때 제목에는 가운데 200pt 를 남기고 긴 성경 이름은 그 폭에 맞춰 글자를 줄인다 — 장을 넘겨도 광고 표시 여부가 바뀌지 않는다. 사이에 184pt 가 들어가지 않으면(분할 화면 등) 광고를 두지 않는다. 왼손 모드에서도 `본문 설정` 을 오른쪽에 두므로 서재 쪽 버튼은 하나이고 광고가 나온다.
- **사이드바 하단:** 카드 높이 124pt. 배지 · 아이콘 44pt · 제목 두 줄 · 본문 한 줄 · CTA. 차트 · 설정 버튼과 24pt, 성경 목록과 16pt 떨어뜨렸다 — 버튼 · 목록을 누르다 광고를 누르지 않게 한다.
- **차트 스폰서:** 타일 그리드 아래 한 줄 전체 폭 가로형 카드(높이 148pt). 16:9 미디어 120pt(이미지 · 동영상) · 배지 · 아이콘 · 제목 두 줄 · 본문 · CTA. 타일 한 칸에 들어가던 세로형 카드는 미디어가 278 × 43.5pt 로 눌려 AdMob 이 「120 × 120 보다 작은 미디어 자리는 수익에서 뺀다」고 경고해 옮겼다. 가로형 소재를 선호한다고 요청한다(보장은 아님).
- **미디어:** 이미지 · 동영상은 차트 카드에서만 그린다. 사이드바 · 헤더는 대표 이미지도 그리지 않고 아이콘 · 글자만 쓴다. 동영상만 골라 빼는 요청 옵션은 SDK 에 없어 **AdMob 콘솔의 광고 단위 미디어 유형으로 정했다(2026-09-14, 사용자 설정)** — 헤더 · 사이드바 단위는 이미지만, 차트 단위는 이미지 · 동영상. 헤더 · 사이드바는 이미지 소재여도 대표 이미지를 그리지 않는다(보여 줄 때만 필수라 정책상 문제없음). Debug 는 모든 자리가 Google 테스트 단위를 함께 써서 헤더 · 사이드바에도 동영상 소재가 올 수 있다.
- **상태:** 받는 동안 자리를 비워 두고, 실패하면 없앤다. 네이티브 광고는 받은 지 1시간이 지나면 만료된다. 헤더는 바로 새로 받되 새 광고가 올 때까지 이전 광고를 두고, 사이드바는 비웠다가 다음에 열 때 받는다.
- **광고 단위:** 차트 · 사이드바 · 헤더를 따로 둔다. Debug 는 Google 테스트 단위를 쓴다.
- 위 SVG(K2 320 × 50, 세로 숨김 제안)는 결정 이전 모습이며 고치지 않았다.
...
**축소 높이 78pt 는 상태바 24pt 기준이다 (2026-09-14 수정).** 버튼 줄은 상태바 아래 10pt 에 44pt 로 놓이므로 78 = 24 + 10 + 44 다. iPadOS 26 의 iPad mini 는 상태바가 32pt 라 버튼 · 헤더 광고가 헤더 아래로 약 8pt 삐져나왔다. 축소 높이를 `max(78, 상태바 + 54)` 로 바꿔 그런 기기에서는 86pt 까지 그린다. 펼친 높이 118pt(본문 시작점)와 축소 진행 계산은 그대로다.
...
- 사이드바 하단의 `개역개정` 은 동작이 아니라 현재 값이라 글자로 남겼다. 아이콘은 어떤 번역본인지 말할 수 없다.
...
- **드래그앤드랍 권장:** `assets/figma-outlined/*.svg` 39개. 필기 샘플 75줄을 실제 **Nanum Pen Script(나눔 펜)** 글꼴의 벡터 윤곽선으로 변환했다. 필기 글꼴이 설치되지 않아도 같은 모양으로 들어간다. UI와 성경 원문은 텍스트다.
...
- **왼손 설정:** [추가 시안](assets/figma-outlined/d3-left-text-settings.svg). 서재는 왼쪽에 유지하고 본문 설정만 그 옆에서 열린다. 이전·다음은 오른쪽에 유지한다. 사이드바 전체를 반전하지 않는다. 팝오버를 연 상태에서 왼손 설정을 바꿀 때는 위치가 갑자기 이동하지 않도록 닫은 뒤 재배치하는 방식을 구현 시 검토한다. → 2026-09-14 부터 본문 설정도 오른쪽 고정이라 재배치가 없다(8-4).
...
기존 33개와 추가 I1 안내 5개의 내보내기 및 PNG 렌더를 확인했다. Figma 실제 드래그앤드랍 결과, 실기기 터치와 애니메이션은 미검증이다. 렌더 검증은 앱 동작 검증을 대신하지 않는다.
...
- **글자 크기:** 설명·힌트·날짜가 3-2 의 "UI 주로 14pt" 보다 작게 그려져 있다. 기존 편집 원본 33개를 XML로 집계한 기준으로는 전체 텍스트 1,516개 중 12pt 이하가 347개(약 22.9%), 11pt 이하가 77개다. 이 수치는 상태바·보드 설명·숨긴 글자도 포함해 순수 UI 설명만의 비율은 아니다. 설명·힌트는 13pt, 목록 날짜는 15pt 부터 비교하고 Dynamic Type 에서 어디까지 커지는지 확인한다.
...
| 큰 글꼴·좁은 창 | 큰 글꼴은 초기 보드 예시, 최신 39개 프레임에는 독립 큰 글꼴 화면이 없음 | 현행 버튼·팔레트 기준 큰 글꼴과 좁은 창 시안 추가 검토 |
...
**결정 (2026-09-14).** 필사 화면 헤더 줄 좌측 광고를 넣는다 — 필사하는 동안 헤더가 화면에 머무는 시간이 길다(사용자 결정). 광고 제거 구매도 붙이고, 광고 화면 다음에 구현한다. 수치와 동작은 6-2 안 K 의 구현 절에 적었다. 로드맵 §4 의 「필사 중 상시 배너 제외」 문장도 함께 고쳤다.

⚠️ **AdMob 배치 가이드와 겹치는 점이 남는다.** 내비게이션 버튼 옆, 사용자가 계속 조작하는 화면, 스크롤에 따라 크기가 바뀌는 광고(56 ↔ 44pt)는 오클릭 · 정책 위반 사례로 적혀 있다([배치 가이드](https://support.google.com/admob/answer/6275345), [네이티브 광고 정책](https://support.google.com/admob/answer/6329638)). 줄인 것 — 헤더 버튼과 다른 모양의 카드, 서재 버튼과 16pt 간격, 광고가 바뀌어도 버튼 · 제목이 움직이지 않음, 축소 헤더 안에서만 그림. 경고나 게재 제한이 오면 높이 고정부터 검토한다.
...
| 왼손 모드의 좌우 | 사이드바는 왼쪽 고정(`NavigationSplitView` 제약). `서재` 는 왼쪽, `본문 설정` 도 오른쪽 고정 — 헤더 광고 자리를 위해 필기 열 쪽으로 옮기지 않는다. 왼손 모드에서 팝오버는 원문 열을 덮는다 | 2026-09-14 (09-10 결정 변경) |
| 헤더 축소 | 스크롤·본문 탭 모두 펼침 ↔ 축소. 완전히 숨기지 않는다. 118pt → 78pt(상태바 24pt 기준. 상태바가 더 높으면 상태바 + 54pt, 2026-09-14), 버튼 시각 크기 86%·터치 영역 44pt. 측정 높이는 펼친 상태로 고정하고 광고 표시 공간을 유지한다 | 2026-09-11 |
| 필사 화면 헤더 줄 광고 | 넣는다. 네이티브. 제목은 늘 가운데이고 광고는 서재 쪽 버튼 뒤에 184pt 고정 폭(가로에서도 같음). 높이는 헤더 축소를 따라 56 ↔ 44pt. 184pt 가 들어가지 않으면 숨긴다(6-2 안 K 구현 절) | 2026-09-14 |
| 광고 제거 구매 | 붙인다. 광고 화면 다음에 구현한다. 설정 한 줄과 구매 복원만 두고 권유 팝업은 없다(안 K 네 번째 프레임) | 2026-09-14 |
| 광고 동의 | UMP. 동의가 필요한 지역에서만 폼을 띄우고, 설정에 광고 개인정보 옵션을 둔다 | 2026-09-14 |
| 광고 미디어 유형 | AdMob 콘솔 광고 단위 설정. 헤더 · 사이드바는 이미지만, 차트는 이미지 · 동영상. 차트 스폰서는 16:9 미디어 213 × 120pt 가로형 카드(6-2 안 K 구현 절) | 2026-09-14 |
...
- **저장소 대조.** SVG 총 106개: 보드 17개·아이콘 11개·편집 원본 39개·가져오기용 39개. 원본과 가져오기용의 파일명·화면 크기를 대조하고 XML 파싱을 확인했다.
- **렌더 확인 이력.** 앞선 SVG 수정 작업에서 가져오기용 33개를 PNG로 렌더해 확인했다. 필기 63줄은 동봉한 Nanum Pen Script 글리프 윤곽선이며 글꼴 대체에 의존하지 않는다. 이번 갱신에서는 I1 기반 안내 5종을 추가로 PNG 렌더해 확인했다. 새 시안의 필기 10줄도 기존 I1의 윤곽선을 그대로 사용했다. Figma 가져오기 결과를 새로 확인한 것은 아니다.
...
- **제안(구현·검증 전).** 절 이미지 배치 · 저장 결과 알림 형태 · 차트의 주 이동 버튼과 타일 문구 정리 · 접근성 요약 문장 · 최초 안내 팝업의 실제 배치 · 광고 자리 · 토큰 이름 매핑.

앱 코드·의존성·빌드 설정은 변경하지 않았다.

### 이전 I1 네 단계 시안 검증 이력

펜 필기·한 손가락을 포함한 손가락 스크롤, 필기 도구, 성경 이동, 절 롱탭 순서로 SVG와 미리보기를 갱신했다. 편집용·가져오기용의 단계 번호와 XML 구조를 확인하고 당시 네 화면을 PNG로 렌더해 검토했다. 이전 이미지·위젯 비교 시안을 포함해 원본과 가져오기용은 각각 39개다. 앱 코드와 실제 터치·애니메이션은 이번 작업 범위에 포함하지 않았다.

### 일반 팝업으로 변경

최초 안내를 단일 팝업의 제목·네 가지 사용법·시작하기 버튼 문구로 정리했다. 이번 변경은 문서에 한하며 기존 SVG와 앱 코드는 수정하지 않았다.
