...
**새로 만드는 개념이 아니다.** 앱에는 이미 `HeaderFeature.showPalatte` 가 있고 헤더의 연필 버튼이 이 값을 토글한다. 기본값이 `false` 라 지금도 팔레트는 접혀 있는 상태로 시작한다. 바뀌는 것은 접힘의 **표현**(헤더 버튼 → 좌측 하단 원형)과 **자동으로 접히는 조건**이다.
...
스크롤을 따라 헤더가 118pt 에서 78pt 로 줄고 번역본 이름이 접힌다. 성경 이름은 26pt 명조에서 16pt 로 작아지고 버튼 줄이 위로 붙는다. 도구 팔레트도 같은 신호로 함께 접히므로 위아래가 같이 물러난다(4-1).
...
**새로 만드는 것이 아니다.** `HeaderFeature.headerAnimation` 이 이미 스크롤 방향을 보고 `headerOffset` 을 0 과 `-headerHeight` 사이에서 움직이며 `HeaderView` 가 그만큼 밀어낸다. 지금은 통째로 사라지는 방식이라 축소가 아니다. 그리고 `showOnlyTitle` 상태가 이미 있으나 아무도 켜지 않는다 — 이름은 "제목만" 인데 코드는 버튼을 숨기므로, 축소에 쓰려면 의미를 뒤집거나 새 상태를 둔다.

**헤더 광고와 부딪힌다.** 헤더 줄에 넣기로 검토 중인 광고(안 K)는 축소할 때 함께 접혀야 한다. 읽는 동안 노출이 사라지므로 광고를 넣을지 정할 때 같이 본다.
...
| 헤더 축소 | 스크롤을 따라 118pt → 78pt 로 줄이고 번역본 이름을 접는다. 측정 높이는 펼친 상태로 고정한다 | 2026-09-10 |
