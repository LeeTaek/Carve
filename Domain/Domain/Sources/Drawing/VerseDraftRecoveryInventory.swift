...
public enum VerseDraftRecoveryReason: String, Equatable, Sendable, CaseIterable {
    ...
    case storeMoved
    /// 근거가 지금 환경과 다르다 — 계정이 바뀜 · 모르던 삭제 · K 를 모름 · 다른 계정을 참고하던 확인 전 초안.
    case otherBasis
...
public struct VerseDraftRecoveryEntry: Equatable, Sendable {
    ...
    /// 그 절의 지금 필기 — 비교의 한쪽. 저장소와 대조하지 않은 묶음(다른 계정 · 이 기기 전용)이면 nil.
...
public struct VerseDraftRecoveryInventory: Equatable, Sendable {
    ...
    public init(
        ...
        entries: [VerseDraftRecoveryEntry], unreadChapters: [BibleChapter]
    ) {
        self.scope = scope
...
public struct VerseDraftRecoveryQuery: Sendable {
    ...
    public func inventory(
        ...
        let compares = environment.readableDraftScopes.contains(scope)
        var entries: [VerseDraftRecoveryEntry] = []
        var unread: [BibleChapter] = []
        for chapter in summary.chapters {
            do {
                entries += try await self.entries(chapter: chapter, scope: scope, compares: compares, environment: environment,
        ...
        return VerseDraftRecoveryInventory(
            scope: scope, summary: summary, comparedWithStore: compares,
            entries: entries.sorted(by: Self.newestFirst), unreadChapters: unread
        )
    }
    ...
    private func entries(
        ...
        environment: DrawingEditEnvironment,
        translation: Translation
    ) async throws -> [VerseDraftRecoveryEntry] {
        let drafts = try await reader.drafts(in: scope, chapter: chapter, translation: translation)
        guard compares else {
            return drafts.map { VerseDraftRecoveryEntry(draft: $0, reason: .otherBasis, current: nil) }
        }
